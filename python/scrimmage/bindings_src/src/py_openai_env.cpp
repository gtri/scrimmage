...
ScrimmageOpenAIEnv::ScrimmageOpenAIEnv(const std::string &mission_file,
                                       bool combine_actors,
                                       bool global_sensor,
...
void ScrimmageOpenAIEnv::render(const std::string & /*mode*/) {
  warning_function_(
      "render must be set in scrimmage mission file with the 'enable_gui' attribute.");
}
...
pybind11::object ScrimmageOpenAIEnv::reset() {
  ...
  reset_scrimmage();
  observations_.update_observation(actions_.ext_ctrl_vec().size(), static_obs_space_);
...
void ScrimmageOpenAIEnv::reset_scrimmage() {
  ...
  if (simcontrol_->enable_gui()) {
    ...
    auto viewer_thread_func = [&]() { viewer_->run(); };
  ...
  observations_.create_observation_space(actions_.ext_ctrl_vec().size(), static_obs_space_);
  actions_.create_action_space(observations_.get_combine_actors());
  set_reward_range();
}

void ScrimmageOpenAIEnv::close() { shutdown_sim(); }
...
void ScrimmageOpenAIEnv::filter_ext_ctrl_vec() {
  ...
  actions_.ext_ctrl_vec().erase(
      std::remove_if(actions_.ext_ctrl_vec().begin(), actions_.ext_ctrl_vec().end(), no_parent),
      actions_.ext_ctrl_vec().end());
}

void ScrimmageOpenAIEnv::filter_ext_sensor_vec() {
  auto no_parent = [&](auto &p) { return p[0]->parent() == nullptr; };
  observations_.ext_sensor_vec().erase(
      std::remove_if(
          observations_.ext_sensor_vec().begin(), observations_.ext_sensor_vec().end(), no_parent),
...
info : dict
   ...
   When there is a single learner, these lists have length 1.
      ...
      .def_property("action_space",
                    &ScrimmageOpenAIEnv::get_action_space,
                    &ScrimmageOpenAIEnv::set_action_space)
      .def_property("observation_space",
                    &ScrimmageOpenAIEnv::get_observation_space,
                    &ScrimmageOpenAIEnv::set_observation_space)
      .def_property(
          "observation", &ScrimmageOpenAIEnv::get_observation, &ScrimmageOpenAIEnv::set_observation)
