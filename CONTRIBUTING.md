...
6.  Your pull request will not be accepted unless the commit builds in the
    assosciated dockerfile for the targeted release. If you want to run
    the complete build and tests locally before submitting a pull request, use
    Docker:

         $ cd /path/to/scrimmage
         $ docker build --build-arg PGK_PREFIX=/opt/scrimmage --tag scrimmage/your_branch:latest --file ./ci/dockerfiles/scrimmage_version_docker_file .
