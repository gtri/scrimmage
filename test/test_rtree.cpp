...
void populate_tree_randomly(
    ...
    for (int i = 0; i < num_contacts; i++) {
        ...
        sc::StatePtr state =
            std::make_shared<sc::State>(Eigen::Vector3d(x, y, z), vel, ang_vel, quat);
        c.set_state(state);
    ...
    for (std::list<sc::Contact>::iterator it = contacts.begin(); it != contacts.end(); it++) {
...
TEST(rtree_test, nearest_n_neighbors) {
    ...
    for (; it1 != rtree_neighbors.end() && it2 != slow_neighbors.end(); it1++, it2++) {
