...
TYPED_TEST(GPUTestFixture, TestZeroAllocAlignment) {
    sc::GPUMapBuffer<TypeParam> map_buffer{
        this->queue,
        0,
    ...
    ASSERT_EQ(map_buffer.capacity(), sc::GPUMapBuffer<TypeParam>::DEFAULT_BUFFER_ALIGNMENT);
}
...
TYPED_TEST(GPUTestFixture, TestNonZeroAllocAlignment) {
    ...
    for (std::size_t expected_capacity = buff_align; expected_capacity <= max_capacity;
         expected_capacity *= buff_align) {
