        ...
        auto find_test_mission = [&](const std::string& filename) -> std::optional<fs::path> {
            ...
            if (!fs::exists(filepath) && filepath.extension() == ".xml") {
                ...
                for (auto& dir_ent : dir_it) {
                    ...
                    bool is_xml_file = dir_ent.exists() && dir_ent.is_regular_file()
...
INSTANTIATE_TEST_SUITE_P(
    ...
    testing::Values(
        ...
        CompareMissions{
            ...
            "./test_missions/straight_gpu_threaded.xml"},
        CompareMissions{
            "./test_missions/straight_cpu_mul.xml",
            "./test_missions/straight_gpu_mul.xml"}));
...
TEST_P(TestMotionModels, CompareMotionModelsTrajectories) {
    ...
    boost::optional<std::string> cpu_log_dir =
        scrimmage::run_test(cpu_mission_path.string(), false, false);
    ...
    for (; cpu_frame_it != cpu_frames.end() && gpu_frame_it != gpu_frames.end();
         ++cpu_frame_it, ++gpu_frame_it) {
