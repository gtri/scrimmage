...
std::map<cl_int, std::string> CL_ERROR_MESSAGES = {
    ...
    std::make_pair<cl_int, std::string>(
...
void show_usage(const char* program_name) {
    fprintf(
        stderr,
        "Usage: %s [-p TARGET_PLATFORM] -- PATH/TO/KERNEL [OpenCL Compiler "
        "Arguments]\n\n",
        program_name);
}
...
int main(int argc, char* argv[]) {
    ...
    // std::string opencl_compiler_options = " -I " +
    // kernel_path.parent_path().string() + " ";
    ...
    for (std::string target_platform : target_platforms) {
        ...
        std::string title_string =
            " Checking program compilation for platform: \'" + target_platform + "\' ";
        ...
        if (check_error(err)) {
            ...
            for (auto device_build_log : build_logs) {
                ...
                std::cout << "Unable to build kernel " << kernel_path << " for platform \'"
                          << target_platform << "\'\n"
                          << std::endl;
            ...
            std::cout << kernel_path << " compiled succesfully on platform \'" << target_platform
                      << "\'" << std::endl;
        }
    }
    return 0;
