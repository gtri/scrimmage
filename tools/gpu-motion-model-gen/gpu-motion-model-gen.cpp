...
int main(int argc, char* argv[]) {
    ...
    std::string enum_str = std::accumulate(
        ...
        enums.cend(),
        std::string{},
        [](const std::string lhs, const EnumDef rhs) { return lhs + "\n" + rhs.string(); });

    to_write = std::regex_replace(to_write, std::regex{R"_(\(>>>enums<<<\))_"}, enum_str);

    to_write = std::regex_replace(to_write, std::regex{R"_(\(>>>KERNEL_NAME<<<\))_"}, kernel_name);
