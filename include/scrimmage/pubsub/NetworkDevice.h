 ...
 public:
    ...
    template <
        ...
        class = std::enable_if_t<
            !std::is_same<T, MessageBase>::value && std::is_base_of<MessageBase, T>::value,
            void>>
    ...
    std::list<MessageBasePtr> msgs(bool pop_msgs = true) {
        static_assert(
            ...
            "You are using the old publisher/subscriber interface (i.e., msgs(bool)) in your "
            "plugin. Please use the new subscriber callback interface.");
        std::list<std::shared_ptr<T>> msg_list_cast;
        return msg_list_cast;
    }
