...
class RayTrace : public scrimmage::Sensor {
 public:
  ...
  /**
   * Defines a single ray for the point cloud in terms of azimuth and elevation
   * angles from center
   */
  class PCRay {
   public:
  ...
  class PointCloudWithId : public PointCloud {
   public:
    ...
    int entity_id;
    bool world_frame;  // Whether the point cloud is in the world frame (true)
                       // or sensor frame (false)
  };
  ...
  /** Whether to have the bullet collision plugin automatically run ray tracing
   * for this sensor and send associated messages */
