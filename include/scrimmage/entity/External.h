...
class External {
 public:
    ...
    std::size_t& shape_queue_max_size() {
        return shape_queue_max_size_;
    }
    using ShapeQueue = std::queue<scrimmage_proto::ShapePtr>;
    ShapeQueue& shape_queue() {
        return shape_queue_;
    }
 ...
 public:
    ...
    boost::function<bool(typename RosType::Request&, typename RosType::Response&)> srv_cb(
        const std::string& service_name, scrimmage::Service sc_service_func,
        Sc2RosResFunc sc2ros_response_func) {
        auto ros2sc_req_func = [&](typename RosType::Request& /*ros_req*/) { return 0; };
        return srv_cb<RosType, ScrimmageResponseType>(service_name, sc_service_func,
                                                      ros2sc_req_func, sc2ros_response_func);
    }
    ...
    boost::function<bool(typename RosType::Request&, typename RosType::Response&)> srv_cb(
        const std::string& service_name, scrimmage::Service sc_service_func,
        Ros2ScRequestFunc ros2sc_request_func, Sc2RosResFunc sc2ros_response_func) {
    ...
    void add_srv_client(ros::NodeHandle& nh, const std::string& sc_topic,
                        Sc2RosReqFunc sc2ros_request_func, Ros2ScResFunc ros2sc_response_func,
        ...
        auto call_service = [__copy_capture_with_this](scrimmage::MessageBasePtr sc_req,
            ...
            RosType srv;
            create_ros_req<ScReqType, Sc2RosReqFunc, RosType>(sc_req, sc2ros_request_func, suffix,
                                                              srv);
            ...
            auto sc_res_cast =
                std::make_shared<Message<ScResType>>(ros2sc_response_func(srv.response));
 ...
 protected:
    ...
    create_ros_req(
        typename std::enable_if_t<std::is_void<ScReqType>::value, MessageBasePtr> /*sc_req*/,
    ...
    void create_ros_req(
        ...
        if (!sc_req_cast) {
            std::cout << "could not cast scrimmage::MessageBase request "
                      ...
                      << boost::typeindex::type_id<ScReqType>().pretty_name() << "> " << err_msg
                      << std::endl;
        }
