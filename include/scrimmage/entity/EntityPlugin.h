...
class EntityPlugin : public Plugin {
 public:
    ...
    SubscriberBasePtr subscribe(const std::string& network_name, const std::string& topic,
                                CallbackFunc callback) {
        SubscriberBasePtr sub =
            pubsub_->subscribe<T>(network_name, topic, callback, 0, false,
    ...
    template <class T, class CallbackFunc>
    SubscriberBasePtr subscribe(const std::string& network_name, const std::string& topic,
                                CallbackFunc callback, unsigned int max_queue_size) {
        SubscriberBasePtr sub =
            pubsub_->subscribe<T>(network_name, topic, callback, max_queue_size, true,
                                  std::static_pointer_cast<EntityPlugin>(shared_from_this()));
        subs_.push_back(sub);
        return sub;
    }
    ...
    void set_pubsub(PubSubPtr pubsub) {
        pubsub_ = pubsub;
    }
    PubSubPtr pubsub() {
        return pubsub_;
    ...
    std::list<SubscriberBasePtr> subs() {
        return subs_;
    }

    void set_time(const std::shared_ptr<Time>& time) {
        time_ = time;
    }
    // cppcheck-suppress passedByValue
    void set_time(std::shared_ptr<const Time> time) {
        time_ = time;
    }
    std::shared_ptr<const Time> getTime() {
        return time_;
    }
    ...
    // loop rate control
    const double& loop_rate() const {
        return loop_rate_;
    }
    const double& loop_timer() const {
        return loop_timer_;
    }
    void set_loop_rate(const double& loop_rate) {
        loop_rate_ = loop_rate;
    }
    void set_loop_timer(const double& loop_timer) {
