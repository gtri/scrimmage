...
class GPUMotionModelImplementation : public GPUMotionModel {
 ...
 public:
    ...
    void init_new_entities(double time) {
        // "Propagate" the motion model with dt=0. This should not update the
        // motion, but should ensure that all states are properly initalized.
        if (to_init_.size() > 0) {
 ...
 protected:
    void collect_states(
        ...
        GPUMapBuffer<T>& states,
        GPUMapBuffer<T>& inputs) {
        ...
        for (auto entityptr_it = entities.cbegin(); entityptr_it != entities.cend();
             ++entityptr_it) {
    ...
    void distribute_states(std::vector<EntityPtr>& entities, GPUMapBuffer<T>& states) {
        ...
        for (auto entity_ptr_it = entities.begin(); entity_ptr_it != entities.end();
             ++entity_ptr_it) {
            entity_ind = std::distance(entities.begin(), entity_ptr_it);
            StatePtr state = (*entity_ptr_it)->state_truth();
            state->pos() << state_info(entity_ind, X), state_info(entity_ind, Y),
                state_info(entity_ind, Z);
            state->vel() << state_info(entity_ind, X_VEL), state_info(entity_ind, Y_VEL),
                state_info(entity_ind, Z_VEL);
            state->ang_vel() << state_info(entity_ind, X_ANG_VEL),
                state_info(entity_ind, Y_ANG_VEL), state_info(entity_ind, Z_ANG_VEL);
