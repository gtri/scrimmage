...
class GPUMapBuffer {
  ...
  std::size_t device_buffer_size() {
    return device_buffer_.getInfo<CL_MEM_SIZE>();
  }

  void push_back(T element) {
    if (size_ == capacity_*sizeof(T)) {
      throw std::out_of_range{"No more room left in buffer to add element. Try resizing buffer to correct size."};
    }
  ...
  iterator insert(const_iterator pos, InputIterator start, InputIterator end) {
    ...
    if (sizeof(T)*(num_additional + size_) > capacity_) {
      throw std::out_of_range{"Tried to add more elements to MapBuffer than MapBuffer has capacity for."};
    }
  ...
  bool unmap() { 
    return _unmap_buffer() == CL_SUCCESS;
  };

  // Returns the pointer to the underlying host_buffer_
  const T* host_buffer() const {
    return host_buffer();
  }
  ...
  const cl::Buffer& device_buffer() const {
  ...
  T& at(std::size_t index) {
    if(host_buffer_ == nullptr || index >= capacity_) {
      throw std::out_of_range{"GPUMapBuffer either has not been mapped or trying to access memory outside its range."};
    }
  ...
  private:
    ...
    std::size_t next_largest_alignment_size(std::size_t bytes) {
      if (bytes == 0) { return buffer_alignment_; }
