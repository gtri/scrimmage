...
class GPUMapBuffer {
 public:
    ...
    void push_back(T element) {
        if (size_ == capacity_ * sizeof(T)) {
            throw std::out_of_range{
                "No more room left in buffer to add element. Try "
                "resizing buffer to correct size."};
        }
    ...
    iterator insert(const_iterator pos, InputIterator start, InputIterator end) {
        ...
        if (sizeof(T) * (num_additional + size_) > capacity_) {
            throw std::out_of_range{
                "Tried to add more elements to MapBuffer than "
                "MapBuffer has capacity for."};
        }
    ...
    bool map(cl_mem_flags map_flags) {
        ...
        if (host_buffer_ != nullptr) {
            std::cerr << "GPUMapBuffer is already mapped into memory, and will not "
                         "be remapped. Did you "
                         "forget to unmap the buffer?\n";
            success = false;
        } else if (
            device_buffer_.get() == nullptr || device_buffer_.getInfo<CL_MEM_SIZE>() != capacity_) {
    ...
    T& at(std::size_t index) {
        if (host_buffer_ == nullptr || index >= capacity_) {
            throw std::out_of_range{
                "GPUMapBuffer either has not been mapped or "
                "trying to access memory outside its "
                "range."};
        }
    ...
    static constexpr std::size_t DEFAULT_BUFFER_ALIGNMENT = 128;
 ...
 private:
    ...
    /*
     * _map_buffer and _unmap_buffer are where device memory gets mapped/unmapped
     * into host memory. N.B. The size of the buffer is set to the capacity of the
     * device_buffer when it is mapped into memory with the flag
     * CL_MAP_WRITE_INVALIDATE_REGION. This invalidates anything in the memory
     * region we are writing to. As such, this acts as though we are creating a
     * new host buffer.
     *
     */
