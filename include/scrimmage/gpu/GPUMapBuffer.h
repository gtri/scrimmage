...
class GPUMapBuffer {
 public:
    ...
    bool map(cl_mem_flags map_flags) {
        ...
        if (host_buffer_ != nullptr) {
            std::cerr
                << "GPUMapBuffer is already mapped into memory, and will not be remapped. Did you "
    ...
    T& at(std::size_t index) {
        if (host_buffer_ == nullptr || index >= capacity_) {
            throw std::out_of_range{
                "GPUMapBuffer either has not been mapped or trying to access memory outside its "
                "range."};
        }
 ...
 private:
    ...
    /*
     * _map_buffer and _unmap_buffer are where device memory gets mapped/unmapped into host memory.
     * N.B. The size of the buffer is set to the capacity of the device_buffer when it is mapped
     * into memory with the flag CL_MAP_WRITE_INVALIDATE_REGION. This invalidates anything 
     * in the memory region we are writing to. As such, this acts as though we are creating a 
     * new host buffer.
     *
     */
