...
struct OpenCLUtils {
  ...
  static bool supports_fp64(const cl::Device& device) {
    cl_int err;
    constexpr cl_device_fp_config MIN_FP_CAPABILITIES = (CL_FP_FMA 
...
#define CL_CHECK_ERROR(err, msg) OpenCLUtils::check_error(err, __FILE__, __LINE__, (msg));
  ...
  static bool check_error(cl_int err, const std::string&& file, const unsigned int linenumber, const std::string&& msg) {
  ...
  static bool check_error(cl_int err, const std::string&& msg) {
    if (err != 0) {
      fprintf(stderr, "OpenCL Error: %s (CODE: %d)\n%s\n",
          OpenCLUtils::CL_ERROR_MESSAGES.at(err).c_str(), err, msg.c_str());
      return true;
    }
    return false;
  };

  static std::optional<std::size_t> cacheline_size(const cl::Device& device) {
