...
bool get_vec_of_vecs(
    ...
    for (auto& vec : out) {
        vec.erase(
            std::remove_if(vec.begin(), vec.end(), [](auto& str) { return str.empty(); }),
            vec.end());
    }

    // Remove any empty vectors from the out vector of vectors
    out.erase(
        std::remove_if(out.begin(), out.end(), [](auto& vec) { return vec.empty(); }),
        out.end());
