...
bool find_terrain_files(
    std::string terrain_name, ConfigParse &terrain_parse,
    ...
    if (terrain_parse.parse(overrides, terrain_name, "SCRIMMAGE_DATA_PATH",
                            file_search)) {
        std::string polydata_file = terrain_parse.directory() + "/" +
                                    terrain_parse.params()["polydata"];
        std::string texture_file =
            terrain_parse.directory() + "/" + terrain_parse.params()["texture"];
        std::string extrusion_file = terrain_parse.directory() + "/" +
                                     terrain_parse.params()["extrusion"];
        std::string extrusion_property =
            terrain_parse.params()["extrusion_property"];
        ...
        if (fs::exists(polydata_file) && fs::exists(texture_file) &&
            fs::is_regular_file(polydata_file) &&
            fs::is_regular_file(texture_file)) {
...
bool find_model_properties(std::string model_name, ConfigParse &cv_parse,
    ...
    if (cv_parse.parse(overrides, model_name, "SCRIMMAGE_DATA_PATH",
                       file_search)) {
        std::string model_file =
            cv_parse.directory() + "/" + cv_parse.params()["model"];
        std::string texture_file =
            cv_parse.directory() + "/" + cv_parse.params()["texture"];
...
unsigned int parse_plugin_vector(
    const std::string &key, std::map<std::string, std::string> &params,
    ...
    for (std::vector<std::string> vecs : vecs_of_vecs) {
        ...
        plugin_overrides_list.push_back(
            PluginOverrides(plugin_name, plugin_overrides));
    }
