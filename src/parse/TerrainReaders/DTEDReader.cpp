...
namespace scrimmage {
  ...
  DTEDReader::DTEDReader(std::string filename): filename_(filename) {
    GDALAllRegister();
  }
  ...
  std::unique_ptr<GDALDataset, GDALDatasetUniquePtrReleaser>  DTEDReader::OpenDTED() const {
    ...
    //GDALDatasetH dataset = GDALOpen(filename_.c_str(), GA_ReadOnly);
    //GDALClose(dataset); 
    std::unique_ptr<GDALDataset, GDALDatasetUniquePtrReleaser> dataset(
        GDALDataset::FromHandle(
          GDALOpenShared(filename_.c_str(), GA_ReadOnly)), GDALDatasetUniquePtrReleaser{});
    return dataset;
  }


  std::unique_ptr<std::array<std::vector<double>, 3>>
    DTEDReader::ReadDTED(GDALDataset& dataset) const {
  ...
  std::unique_ptr<common::ElevationGrid> DTEDReader::Parse() const {
    ...
    if(!elevation_grid) {
      std::cout << "Unable to Read DTED file \'" << filename_ << "\'\n";
    }
  ...
  std::unique_ptr<std::array<std::vector<double>, 3>> DTEDReader::ParseAsUTM(
    ...
    if(!elevation_data) {
      std::cout << "Unable to open or read DTED file \'" << filename_ << "\'\n";
    }
