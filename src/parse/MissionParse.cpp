...
namespace scrimmage {
  ...
  template<class Parser>
    bool MissionParse::parse_mission() {
      ...
      for (auto attr = run_node.first_attribute();
          attr.is_valid(); attr = attr.next()) {
      ...
      auto parse_tags = [&](
        ...
        for (auto node = runscript_node.first_node(tagname);
            node.is_valid(); node = node.next_sibling(tagname)) {
      ...
      for (auto script_node = runscript_node.first_node("param_common");
          script_node.is_valid();
        ...
        for (auto node = script_node.first_node(); node.is_valid();
      ...
      attributes_.clear();
      for (auto node = runscript_node.first_node(); node.is_valid();
          node = node.next_sibling()) {
      ...
      for (auto script_node = runscript_node.first_node("entity_common");
          script_node.is_valid();
      ...
      for (auto script_node = runscript_node.first_node("entity");
          script_node.is_valid();
        ...
        for (auto base_node = script_node.first_node("base");
            base_node.is_valid();
