...
bool MissionParse::read_file_content(const std::string &filename) {
  ...
  if (!fs::exists(mission_filename_)) {
    ...
    std::string result = "";
    bool status =
        file_search.find_file(mission_filename_, "xml", "SCRIMMAGE_MISSION_PATH", result, false);
    if (!status) {
...
bool MissionParse::parse_terrain() {
  ...
  if (params_.count("terrain") > 0 &&
    ...
    GeographicLib::UTMUPS::Forward(
        latitude_origin(), longitude_origin(), zone, northp, x_easting, y_northing, gamma, k_scale);
    ...
    if (((northp && boost::to_upper_copy(terrain_parse.params()["hemisphere"]) == "NORTH") ||
      ...
      double terrain_min_x = -std::numeric_limits<double>::max();
      double terrain_max_x = std::numeric_limits<double>::max();
      double terrain_min_y = -std::numeric_limits<double>::max();
      double terrain_max_y = std::numeric_limits<double>::max();
      // This is ugly
      if (terrain_parse.params().count("terrain_extent") == 1) {
        if (terrain_parse.params().count("terrain_extent/xmin") == 1) {
          terrain_min_x = std::stod(terrain_parse.params()["terrain_extent/xmin"]);
        }
        if (terrain_parse.params().count("terrain_extent/xmax") == 1) {
          terrain_max_x = std::stod(terrain_parse.params()["terrain_extent/xmax"]);
        }
        if (terrain_parse.params().count("terrain_extent/ymin") == 1) {
          terrain_min_y = std::stod(terrain_parse.params()["terrain_extent/ymin"]);
        }
        if (terrain_parse.params().count("terrain_extent/ymax") == 1) {
          terrain_max_y = std::stod(terrain_parse.params()["terrain_extent/ymax"]);
        }
      }

      assert(terrain_min_x <= terrain_max_x);
      ...
      auto terrain_extent = utm_terrain_->mutable_terrain_extent();
      terrain_extent->set_x_min(terrain_min_x);
      terrain_extent->set_x_max(terrain_max_x);
      terrain_extent->set_y_min(terrain_min_y);
      terrain_extent->set_y_max(terrain_max_y);
      return true;
