...
bool MissionParse::parse(const std::string &filename) {
  ...
  try {
    ...
    cout << "scrimmage::MissionParse::parse: Exception during "
  ...
  for (rx::xml_node<> *script_node = runscript_node->first_node("param_common");
    ...
    if (nm_attr == 0) {
      std::cout << "warning: found param_common block without a name, skipping"
                << std::endl;
      continue;
    }
  ...
  for (rapidxml::xml_node<> *script_node =
    ...
    if (nm_attr == 0) {
      cout << "warning: found entity_common block without a name, skipping"
           << endl;
      continue;
    }
  ...
  for (rapidxml::xml_node<> *script_node = runscript_node->first_node("entity");
    ...
    if (nm_attr != 0) {
      ...
      if (it == entity_common.end()) {
        cout << "warning: entity_common block referenced without definition"
             << endl;
      } else {
...
bool MissionParse::parse_terrain() {
  ...
  if (params_.count("terrain") > 0 &&
    ...
    GeographicLib::UTMUPS::Forward(latitude_origin(), longitude_origin(), zone,
                                   northp, x_easting, y_northing, gamma,
                                   k_scale);
    ...
    if (((northp && boost::to_upper_copy(
         ...
         (!northp && boost::to_upper_copy(
                         terrain_parse.params()["hemisphere"]) == "SOUTH")) &&
