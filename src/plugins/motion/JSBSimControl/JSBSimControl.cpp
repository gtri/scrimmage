...
bool JSBSimControl::init(
    ...
    Eigen::Vector3d lla;
    parent_->projection()
        ->Reverse(state_->pos()[0], state_->pos()[1], state_->pos()[2], lla[0], lla[1], lla[2]);
    ...
    Eigen::Vector3d a_FLU(
        ax_pilot_node_->getDoubleValue(),
...
bool JSBSimControl::step(double time, double dt) {
    ...
    Eigen::Vector3d a_FLU(
        ax_pilot_node_->getDoubleValue() * feet2meters,
