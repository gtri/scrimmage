...
bool SingleIntegrator::init(std::map<std::string, std::string> &info, std::map<std::string, std::string> &params) {
...
bool SingleIntegrator::step(double /*t*/, double dt) {
    Eigen::Vector3d desired_vel(vars_.input(vel_x_idx_), vars_.input(vel_y_idx_), vars_.input(vel_z_idx_));
