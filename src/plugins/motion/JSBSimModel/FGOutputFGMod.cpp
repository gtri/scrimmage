...
FGOutputFGMod::FGOutputFGMod(FGFDMExec* fdmex) : FGOutputFG(fdmex) {
    ...
    if (fdmex->GetDebugLevel() > 0) {
        ...
        if (Propulsion->GetNumEngines() > FG_MAX_ENGINES)
            LOG_WARN("This vehicle has " << Propulsion->GetNumEngines()
                     << " engines, but the current version of FlightGear's FGNetFDM only supports "
                     << FG_MAX_ENGINES << " engines. Only the first " << FG_MAX_ENGINES
                     << " engines will be used.");

        // Consumables
        if (Propulsion->GetNumTanks() > FG_MAX_TANKS)
            LOG_WARN("This vehicle has " << Propulsion->GetNumTanks()
                     << " tanks, but the current version of FlightGear's FGNetFDM only supports "
                     << FG_MAX_TANKS << " tanks. Only the first " << FG_MAX_TANKS
                     << " tanks will be used.");

        // Gear status
        if (GroundReactions->GetNumGearUnits() > FG_MAX_WHEELS)
            LOG_WARN("This vehicle has " << GroundReactions->GetNumGearUnits()
                     << " bogeys, but the current version of FlightGear's FGNetFDM only supports "
                     << FG_MAX_WHEELS << " bogeys. Only the first " << FG_MAX_WHEELS
                     << " bogeys will be used.");
    }
}
...
void FGOutputFGMod::PacketInit(FGPacketVersion version) {
    switch (version) {
        case FGPacketVersion::v24: {
            dataLength = sizeof(FGNetFDM1)
                         + sizeof(FGNetFDM3);  // FS 2020.2 or earlier does not use FGNetFDM2
...
void FGOutputFGMod::SocketDataFillMod(void) {
    ...
    net1->thetadot =
        static_cast<float>(Auxiliary->GetEulerRates(eTht));             // pitch rate (radians/sec)
    ...
    net1->slip_deg =
        static_cast<float>(Auxiliary->Getbeta(inDegrees));  // slip ball deflection, deg

    net1->num_engines = std::min(
        static_cast<size_t>(FG_MAX_ENGINES),
    ...
    net1->num_tanks = std::min(
        static_cast<size_t>(FG_MAX_TANKS),
    ...
    net3->num_wheels =
        std::min(static_cast<int>(FG_MAX_WHEELS), GroundReactions->GetNumGearUnits());
    ...
    // Environment
    net3->cur_time = static_cast<uint64_t>(
