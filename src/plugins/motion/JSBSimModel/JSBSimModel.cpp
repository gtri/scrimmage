...
REGISTER_PLUGIN(scrimmage::MotionModel, scrimmage::motion::JSBSimModel,
                JSBSimModel_plugin)
...
bool JSBSimModel::init(std::map<std::string, std::string> &info,
    ...
    roll_idx_ = vars_.declare(VariableIO::Type::desired_roll,
                              VariableIO::Direction::In);
...
bool JSBSimModel::step(double time, double dt) {
    ...
    if (use_pitch_) {
        ...
        // Try to remove altitude control by putting setpoint to current
        // altitude
