...
bool JSBSimModel::init(
    ...
    std::string z_name = use_pitch_ ? vars_.type_map().at(VariableIO::Type::desired_pitch)
...
void JSBSimModel::teleport(StatePtr& state) {
    double lat, lon, alt;
    parent_->projection()
        ->Reverse(state->pos()(0), state->pos()(1), state->pos()(2), lat, lon, alt);
