...
bool UUV6DOF::init(std::map<std::string, std::string> &info,
  ...
  //         for (std::vector<std::string> vec : vecs) {
  //             if (vec.size() != 6) {
  //                 cout << "Invalid vector size in: " << added_mass_matrix <<
  //                 endl; break;
  //             }
  //             for (int i = 0; i < 6; i++) {
  ...
  masses_ << m - Xu_dot, 0, 0, 0, m * zg, -m * yg, 0, m - Yv_dot, 0, -m * zg, 0,
      m * xg - Yr_dot, 0, 0, m - Zw_dot, m * yg, -m * xg - Zq_dot, 0, 0,
...
void UUV6DOF::model(const vector_t &x, vector_t &dxdt, double t) {
  ...
  double X_ext = F_hydro(0) + Xuu_ * x[U] * std::abs(x[U]) + Xu_dot * x[U_dot] +
                 Xwq * x[W] * x[Q] + Xqq * x[Q] * x[Q] + Xvr * x[V] * x[R] +
                 Xrr * x[R] * x[R] + Xprop;
