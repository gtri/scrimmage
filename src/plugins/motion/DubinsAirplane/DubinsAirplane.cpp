...
bool DubinsAirplane::init(std::map<std::string, std::string> &info,
  ...
  desired_pitch_idx_ =
      vars_.declare(VariableIO::Type::desired_pitch, VariableIO::Direction::In);
  desired_roll_idx_ =
...
bool DubinsAirplane::step(double t, double dt) {
  ...
  speed_ = boost::algorithm::clamp(vars_.input(desired_speed_idx_), speed_min_,
                                   speed_max_);
  pitch_ = boost::algorithm::clamp(vars_.input(desired_pitch_idx_), pitch_min_,
                                   pitch_max_);
