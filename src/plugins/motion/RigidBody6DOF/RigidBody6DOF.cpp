...
bool RigidBody6DOF::init(
    std::map<std::string, std::string>& info,
    std::map<std::string, std::string>& params) {
    x_.resize(MODEL_NUM_ITEMS);
    Eigen::Vector3d pos = parent()->state_belief()->pos();
    quat_world_ = parent()->state_belief()->quat_const();
...
bool RigidBody6DOF::step(double time, double dt) {
    ...
    auto new_quat = quat_world_ * quat_local_;
    parent()->state_belief()->quat() = new_quat;
    parent()->state_belief()->pos() = Eigen::Vector3d(x_[Xw], x_[Yw], x_[Zw]);
