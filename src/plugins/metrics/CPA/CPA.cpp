...
bool CPA::step_metrics(double t, double dt) {
  ...
  if (!outside_time) {
    ...
    for (auto &kv : *id_to_ent_map_) {
      // Checks if a specific team ID is not being checked or if the entity has
      // the specific team ID
      if ((team_id_ <= 0) || (kv.second->id().team_id() == team_id_)) {
        for (auto &kv2 : *id_to_ent_map_) {
          if (kv != kv2) {
            // Checks if teams are not being checked or if the teams are the
            // same
            if (!team_cpa_ ||
...
void CPA::calc_team_scores() {
  for (auto &kv : cpa_map_) {
    csv_.append(
        scrimmage::CSV::Pairs{{"entity", kv.first},
