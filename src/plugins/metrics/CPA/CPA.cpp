...
REGISTER_PLUGIN(scrimmage::Metrics, scrimmage::metrics::CPA, CPA_plugin)
...
void CPA::init(std::map<std::string, std::string> &params) {
    ...
    // If team_id_ is set to a specific team, then team_cpa_ should be
    // automatically set based on how the checks work below
...
bool CPA::step_metrics(double t, double dt) {
    ...
    bool outside_time = (((min_time_s_ >= 0.0) && (t < min_time_s_)) ||
                         ((max_time_s_ > 0.0) && (t > max_time_s_)));
    if (!outside_time) {
        if (!initialized_) {
            ...
            csv_.set_column_headers(scrimmage::CSV::Headers{
                "entity", "cpa", "closest_entity", "time"});
            initialized_ = true;
        }

        for (auto &kv : *id_to_ent_map_) {
            // Checks if a specific team ID is not being checked or if the
            // entity has the specific team ID
            if ((team_id_ <= 0) || (kv.second->id().team_id() == team_id_)) {
                for (auto &kv2 : *id_to_ent_map_) {
                    if (kv != kv2) {
                        ...
                        // are the same
                        if (!team_cpa_ || (kv.second->id().team_id() ==
                                           kv2.second->id().team_id())) {
                            if (kv.second->state_truth() &&
                                kv2.second->state_truth()) {
                                double cur_distance =
                                    (kv.second->state_truth()->pos() -
                                     kv2.second->state_truth()->pos())
                                        .norm();
                                if (cur_distance <
                                    cpa_map_[kv.first].distance()) {
                                    cpa_map_[kv.first].set_distance(
                                        cur_distance);
                                    cpa_map_[kv.first].set_closest_entity(
                                        kv2.first);
