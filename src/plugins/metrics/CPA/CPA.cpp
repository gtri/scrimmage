...
void CPA::init(std::map<std::string, std::string> &params) {
    ...
    // If team_id_ is set to a specific team, then team_cpa_ should be automatically set based on
    // how the checks work below
...
bool CPA::step_metrics(double t, double dt) {
    ...
    if (!outside_time) {
        ...
        for (auto &kv : *id_to_ent_map_) {
            // Checks if a specific team ID is not being checked or if the entity has the specific
            // team ID
