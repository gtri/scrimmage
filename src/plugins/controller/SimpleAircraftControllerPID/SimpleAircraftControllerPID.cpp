...
REGISTER_PLUGIN(scrimmage::Controller, scrimmage::controller::SimpleAircraftControllerPID,
...
void SimpleAircraftControllerPID::init(std::map<std::string, std::string> &params) {
    ...
    // y_vel^2))
    std::string alt_ctrl_name = use_glide_slope_ ? vars_.type_map().at(VariableIO::Type::desired_glide_slope)
...
bool SimpleAircraftControllerPID::step(double t, double dt) {
    ...
    double u_roll_rate =
        use_roll_ ? -heading_pid_.step(dt, state_->quat().roll()) : heading_pid_.step(dt, state_->quat().yaw());

    alt_pid_.set_setpoint(vars_.input(input_altitude_or_glide_slope_idx_));
    double u_pitch_rate = use_glide_slope_
                              ? alt_pid_.step(dt, atan(state_->vel()(2) / sqrt(state_->vel()(0) * state_->vel()(0) +
