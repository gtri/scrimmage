...
void SimpleAircraftControllerPID::init(
    ...
    std::string ctrl_name =
        use_roll_ ? vars_.type_map().at(VariableIO::Type::desired_roll)
                  : vars_.type_map().at(VariableIO::Type::desired_heading);
    // Set up to use either altitude or glide slope (z_vel / sqrt(x_vel^2 +
    // y_vel^2))
    std::string alt_ctrl_name =
        use_glide_slope_
            ? vars_.type_map().at(VariableIO::Type::desired_glide_slope)
            : vars_.type_map().at(VariableIO::Type::desired_altitude);

    input_roll_or_heading_idx_ =
        vars_.declare(ctrl_name, VariableIO::Direction::In);
    input_altitude_or_glide_slope_idx_ =
        vars_.declare(alt_ctrl_name, VariableIO::Direction::In);
    input_velocity_idx_ = vars_.declare(VariableIO::Type::desired_speed,
                                        VariableIO::Direction::In);

    output_throttle_idx_ =
        vars_.declare(VariableIO::Type::throttle, VariableIO::Direction::Out);
    output_roll_rate_idx_ =
...
bool SimpleAircraftControllerPID::step(double t, double dt) {
    ...
    double u_pitch_rate =
        use_glide_slope_
            ? alt_pid_.step(dt, atan(state_->vel()(2) /
                                     sqrt(state_->vel()(0) * state_->vel()(0) +
                                          state_->vel()(1) * state_->vel()(1))))
