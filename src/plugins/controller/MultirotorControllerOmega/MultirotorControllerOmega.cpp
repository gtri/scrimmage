...
MultirotorControllerOmega::MultirotorControllerOmega()
    : pwm_max_(2000), pwm_min_(1000) {}

void MultirotorControllerOmega::init(
...
bool MultirotorControllerOmega::step(double t, double dt) {
    ...
    if (d_state) {
        ...
        } else if (d_state->input_type() ==
                   sm::MultirotorState::InputType::PWM) {
            u_ = sc::scale(
                d_state->prop_input().head(multirotor_->rotors().size()),
                pwm_min_, pwm_max_, multirotor_->omega_min(),
