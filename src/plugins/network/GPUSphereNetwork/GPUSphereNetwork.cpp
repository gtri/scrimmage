...
bool GPUSphereNetwork::step(
    std::map<std::string, std::list<NetworkDevicePtr>>& pubs,
    ...
    const auto add_pub_subs =
        [&pub_to_subs](const TopicNetworkDevices& pubs, const TopicNetworkDevices& subs) -> void {
        for (const auto& kv : pubs) {
...
bool GPUSphereNetwork::deliver_topic_messages(
    NetworkDevicePtr pub,
...
bool GPUSphereNetwork::is_reachable(
    ...
    if (pub_plugin->parent() == sub_plugin->parent())
        return true;
...
bool GPUSphereNetwork::is_successful_transmission(
    const scrimmage::EntityPluginPtr& pub_plugin,
    const scrimmage::EntityPluginPtr& sub_plugin) {
