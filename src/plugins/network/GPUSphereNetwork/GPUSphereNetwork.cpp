...
bool GPUSphereNetwork::init(std::map<std::string, std::string> &mission_params,
  ...
  std::string kernel_name = sc::get<std::string>("network_kernel", plugin_params, "in_range");
   
  GPUControllerPtr gpu = parent()->gpu_controller();
  MissionParsePtr mp = parent()->mp();
  std::map<std::string, GPUPluginBuildParams> gpu_plugin_params = gpu->get_plugin_params(mp, GPU_PLUGIN_TYPE::NETWORK);
  assert(gpu_plugin_params.count(kernel_name) > 0);
  GPUPluginBuildParams& network_kernel_params = gpu_plugin_params[kernel_name];
...
bool GPUSphereNetwork::step(std::map<std::string, std::list<NetworkDevicePtr>> &pubs,
  ...
  GPUMapBuffer<uint8_t> reachable_map{queue_, (n_ents*n_ents / sizeof(uint8_t)) + 1, CL_MEM_WRITE_ONLY};
  ...
  err = kernel_.setArg(1, reachable_map_.
