...
REGISTER_PLUGIN(scrimmage::Network, scrimmage::network::SphereNetwork,
...
bool SphereNetwork::init(std::map<std::string, std::string> &mission_params,
  ...
  filter_comms_plane_ =
      sc::get<bool>("filter_comms_plane", plugin_params, filter_comms_plane_);
  if (filter_comms_plane_) {
    comms_boundary_altitude_ = sc::get<double>(
        "comms_boundary_altitude", plugin_params, comms_boundary_altitude_);
    comms_boundary_epsilon_ = sc::get<double>(
        "comms_boundary_epsilon", plugin_params, comms_boundary_epsilon_);
  }
  return true;
}
...
bool SphereNetwork::is_reachable(const scrimmage::EntityPluginPtr &pub_plugin,
  ...
  rtree_->neighbors_in_range(pub_plugin->parent()->state_truth()->pos(), neigh,
                             range_);
...
bool SphereNetwork::is_successful_transmission(
...
bool SphereNetwork::within_planar_boundary(double z1, double z2) {
  bool both_above =
      (z1 >= (comms_boundary_altitude_ - comms_boundary_epsilon_)) &&
