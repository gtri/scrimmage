...
bool FlagCaptureInteraction::step_entity_interaction(std::list<sc::EntityPtr>& ents, double t,
                                                     double dt) {
    ...
    if (!flag_taken_) {
        for (sc::EntityPtr ent : ents) {
            if (ent->id().team_id() != flag_boundary_shape_.id().team_id()
                ...
                auto msg = std::make_shared<sc::Message<sm::FlagTaken>>();
        ...
        if (it != id_to_ent_map_->end() && it->second != nullptr
            ...
            if (capture_boundary_->contains(it->second->state_truth()->pos())) {
                ...
                auto msg = std::make_shared<sc::Message<sm::FlagCaptured>>();
