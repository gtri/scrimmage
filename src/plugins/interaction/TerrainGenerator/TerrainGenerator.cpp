...
REGISTER_PLUGIN(scrimmage::EntityInteraction, scrimmage::interaction::TerrainGenerator,
...
bool TerrainGenerator::init(std::map<std::string, std::string>& mission_params,
    ...
    double z_min =
        sc::get<double>("z_min", plugin_params, -std::numeric_limits<double>::infinity());
    double z_max =
        sc::get<double>("z_max", plugin_params, +std::numeric_limits<double>::infinity());
    double z_std = sc::get<double>("z_std", plugin_params, 1.0);

    std::string technique_str = sc::get("technique", plugin_params, "RANDOM_WALK");
    ...
    if (str2container(sc::get<std::string>("center", plugin_params, "0, 0, 0"), ", ", center_vec,
                      3)) {
    ...
    if (str2container(sc::get<std::string>("color", plugin_params, "0, 255, 0"), ", ", color_vec,
                      3)) {
        color = vec2eigen(color_vec);
    }

    map_ =
        TerrainMap(random_->make_rng_normal(0.0, z_std), random_->gener(), technique, center_point,
                   x_length, y_length, x_resolution, y_resolution, z_min, z_max, color);
...
bool TerrainGenerator::step_entity_interaction(std::list<sc::EntityPtr>& ents, double t,
                                               double dt) {
