...
bool TerrainGenerator::init(
    ...
    if (str2container(
            sc::get<std::string>("center", plugin_params, "0, 0, 0"),
            ", ",
            center_vec,
            3)) {
    ...
    if (str2container(
            sc::get<std::string>("color", plugin_params, "0, 255, 0"),
            ", ",
            color_vec,
            3)) {
    ...
    map_ = TerrainMap(
        random_->make_rng_normal(0.0, z_std),
        random_->gener(),
        technique,
        center_point,
        x_length,
...
bool TerrainGenerator::step_entity_interaction(
    std::list<sc::EntityPtr>& ents,
    double t,
    double dt) {
