...
draw_graph(scrimmage_msgs::Graph &graph,
    ...
    for (auto &e : graph.edges()) {
        ...
        set(edge_shape->mutable_line()->mutable_start(), p1.x(), p1.y(),
            p1.z());
    ...
    for (const auto &pt : graph.nodes() | ba::transformed(to_pt)) {
        set(node_shape->mutable_pointcloud()->add_point(), pt.x(), pt.y(),
            pt.z());
    ...
    for (const auto &node : graph.nodes()) {
        ...
        if (draw_node_labels == DrawNodeLabels::YES) {
            Eigen::Vector3d pos(node.point().x(), node.point().y(),
                                node.point().z());
            pos(0) += 5;
...
std::unordered_map<uint64_t, scrimmage_proto::Vector3d> nodes_idxs_to_pos_map(
    ...
    auto to_pos = [&](auto &node) {
        return std::make_pair(node.id(), node.point());
    };
