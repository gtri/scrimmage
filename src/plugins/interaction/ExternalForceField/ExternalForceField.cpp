...
bool ExternalForceField::init(
    ...
    std::string force_type_str =
        sc::get("force_type", plugin_params, "constant");
    if (force_type_str == "constant") {
        ...
        if (str2container(sc::get("constant_force", plugin_params, "0, 0, 0"),
            ...
            // std::cout << "Force is [0]: " << force_vec[0] << ", [1]: " <<
            // force_vec[1] << std::endl;
        ...
        if (str2container(
                sc::get("force_change_period", plugin_params, "0.0, 1.0"), ", ",
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (str2container(sc::get("variable_force_" + std::to_string(i),
                              ...
                              ", ", var_force_vec, 2)) {
                auto force_noise = parent_->random()->make_rng_normal(
                    var_force_vec[0], var_force_vec[1]);
    ...
    pitch_period_ =
        sc::get("moment_pitch_period", plugin_params, pitch_period_);
...
bool ExternalForceField::step_entity_interaction(std::list<sc::EntityPtr> &ents,
    ...
    for (auto &ent : ents) {
        ...
        if (ent->state_truth()->pos()(2) >= moment_enable_min_z_ &&
            ...
            double scale =
                (ent->state_truth()->pos()(2) - moment_enable_min_z_) /
