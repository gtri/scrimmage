...
REGISTER_PLUGIN(scrimmage::EntityInteraction,
                scrimmage::interaction::BulletCollision, BulletCollision_plugin)
...
bool BulletCollision::init(std::map<std::string, std::string> &mission_params,
    ...
    enable_collision_detection_ =
        sc::get<bool>("enable_collision_detection", plugin_params, true);
    enable_ray_tracing_ =
        sc::get<bool>("enable_ray_tracing", plugin_params, true);

    remove_on_collision_ = sc::get<bool>("remove_on_collision", plugin_params,
                                         remove_on_collision_);
    show_collision_shapes_ = sc::get<bool>(
        "show_collision_shapes", plugin_params, show_collision_shapes_);
    ...
    enable_terrain_ =
        sc::get<bool>("enable_terrain", plugin_params, enable_terrain_);

    enable_team_collisions_ = get<bool>("enable_team_collisions", plugin_params,
                                        enable_team_collisions_);
    enable_non_team_collisions_ =
        get<bool>("enable_non_team_collisions", plugin_params,
                  enable_non_team_collisions_);
    ...
    publish_on_local_networks_ = get("publish_on_local_networks", plugin_params,
                                     publish_on_local_networks_);
    pcl_network_name_ =
        get("pcl_network_name", plugin_params, pcl_network_name_);
    pcl_topic_name_ = get("pcl_topic_name", plugin_params, pcl_topic_name_);
    prepend_pcl_topic_with_id_ =
        get("prepend_pcl_topic_with_id", plugin_params, false);
    ...
    auto ent_gen_cb = [&](scrimmage::MessagePtr<sm::EntityGenerated> msg) {
        ...
        btSphereShape *sphere_shape =
            new btSphereShape(ent->radius());  // TODO: memory management
        ...
        if (show_collision_shapes_) {
            objects_[id].shape =
                sc::shape::make_sphere(ent->state_truth()->pos(), ent->radius(),
                ...
                if (kv.second->type() == "Ray") {
                    ...
                    if (rs->automatic_ray_tracing() == true) {
                        ...
                        if (publish_on_local_networks_) {
                            ...
                            pc_desc->pub =
                                plugin->advertise(pcl_network_name_, topic, 10);

                            // Add the empty plugin to the entity's autonomies
                            // list
                            ent->autonomies().push_back(plugin);
                        } else {
                            pc_desc->pub =
                                advertise(pcl_network_name_, topic, 10);
                        }
                        ...
                        if (rs) {
                            ...
                            // Get the rays (copied internally, so can just
                            // assign here), using the model name
                            //  so the dirty flag can be checked later.
                            pcl.rays = rs->rays();
                            for (auto &ray : pcl.rays) {
                                Eigen::Vector3d r(rs->max_range(), 0, 0);
                                sc::Quaternion rot_vert(
                                    Eigen::Vector3d(0, 1, 0),
                                    ray.elevation_rad);
                                r = rot_vert.rotate(r);

                                sc::Quaternion rot_horiz(
                                    Eigen::Vector3d(0, 0, 1), ray.azimuth_rad);
                            ...
                            if (show_rays_) {
                                ...
                                for (unsigned int i = 0;
                                     i < pc_desc->point_cloud.points.size();
                                     i++) {
                                    std::shared_ptr<sp::Shape> line(
                                        new sp::Shape);
                                    sc::set(line->mutable_color(), 255, 0, 0);
                                    line->set_opacity(1.0);
                                    sc::set(
                                        line->mutable_line()->mutable_start(),
    ...
    auto shape_gen_cb = [&](scrimmage::MessagePtr<sp::Shapes> msg) {
        for (int i = 0; i < msg->data.shape_size(); i++) {
            if (msg->data.shape(i).oneof_type_case() == sp::Shape::kCuboid) {
                btVector3 xyz(
                    btScalar(msg->data.shape(i).cuboid().x_length() / 2.0),
                ...
                coll_object->getWorldTransform().setOrigin(btVector3(
                    (btScalar)msg->data.shape(i).cuboid().center().x(),
    ...
    if (enable_ground_plane_) {
        ...
        coll_object->getWorldTransform().setOrigin(btVector3(
            (btScalar)0, (btScalar)0, (btScalar)ground_plane_height_));
        bt_collision_world->addCollisionObject(coll_object);
    }
        ...
        if (utm_terrain->enable_terrain() == true) {
            ...
            if (extension == ".ply") {
                ...
                reader->SetGeometryFileName(
                    utm_terrain->poly_data_file().c_str());
                reader->Update();
                polyData = reader->GetOutput();
            } else {
            ...
            vtkSmartPointer<vtkPolyData> transformedData =
            ...
            triFilter->Update();
            vtkSmartPointer<vtkPolyData> triangularData =
                triFilter->GetOutput();

            // Get the results
            btTriangleMesh *triangles = new btTriangleMesh();
            for (vtkIdType index = 0;
                 index < triangularData->GetNumberOfCells(); index++) {
                ...
                triangles->addTriangle(btVector3(p0[0], p0[1], p0[2]),
                                       btVector3(p1[0], p1[1], p1[2]),
                                       btVector3(p2[0], p2[1], p2[2]));
            }
            btCollisionShape *mesh =
                new btBvhTriangleMeshShape(triangles, true, true);
            btDefaultMotionState *motionState = new btDefaultMotionState(
                btTransform(btQuaternion(0, 0, 0, 1), btVector3(0, 0, 0)));
            btRigidBody::btRigidBodyConstructionInfo rigidBodyConstructionInfo(
                0.0f, motionState, mesh, btVector3(0, 0, 0));
            btRigidBody *rigidBodyTerrain =
...
bool BulletCollision::step_entity_interaction(std::list<sc::EntityPtr> &ents,
    ...
    for (auto &kv : *id_to_ent_map_) {
        ...
        if (it_object != objects_.end()) {
            ...
            if (show_collision_shapes_) {
                sc::set(
                    it_object->second.shape->mutable_sphere()->mutable_center(),
                    ent->state_truth()->pos());
    ...
    for (auto &kv : pc_descs_) {
        ...
        for (auto &kv2 : kv.second) {
            ...
            if (t > (pc.max_sample_rate + kv2.second->last_update_time)) {
                ...
                auto msg =
                    std::make_shared<sc::Message<RayTrace::PointCloud>>();
                ...
                Eigen::Matrix4d tf_m =
                    own_ent->state_truth()->tf_matrix(false) *
                ...
                for (RayTrace::PCPoint &pcpoint : pc.points) {
                    ...
                    Eigen::Vector4d sensor_pos =
                        tf_m * Eigen::Vector4d(0, 0, 0, 1);
                    Eigen::Vector3d sensor_pos_w =
                        sensor_pos.head<3>() + own_pos;

                    // Transform ray's end point to world coordinates
                    Eigen::Vector4d ray =
                        tf_m * Eigen::Vector4d(original_ray(0), original_ray(1),
                                               original_ray(2), 1);
                    Eigen::Vector3d ray_w = ray.head<3>() + own_pos;

                    // Create bullet vectors
                    btVector3 btFrom(sensor_pos_w(0), sensor_pos_w(1),
                                     sensor_pos_w(2));
                    btVector3 btTo(ray_w(0), ray_w(1), ray_w(2));

                    // Perform ray casting
                    btCollisionWorld::ClosestRayResultCallback res(btFrom,
                                                                   btTo);
                    bt_collision_world->rayTest(btFrom, btTo, res);

                    // Points in the RayTrace message are defined with respect
                    // to the LIDAR sensor's coordinate frame. Use original
                    // ray's direction, shorten to length of detection ray, if a
                    // collision occurred.
                    ...
                    if (res.hasHit()) {
                        ...
                        msg->data.points.push_back(
                            RayTrace::PCPoint(original_ray, 255, true));
                        ray_end = ray_w;
                    }

                    if (show_rays_) {
                        std::unique_ptr<PointCloudDescription> &pc_desc =
                            kv2.second;
                        if (res.hasHit()) {
                            sc::set(pc_desc->shapes[i]->mutable_color(), 255, 0,
                                    0);
                            pc_desc->shapes[i]->set_opacity(1.0);
                        } else {
                            sc::set(pc_desc->shapes[i]->mutable_color(), 0, 0,
                                    255);
                            pc_desc->shapes[i]->set_opacity(0.5);
                        }
                        sc::set(
                            pc_desc->shapes[i]->mutable_line()->mutable_start(),
                            sensor_pos_w);
                        sc::set(
                            pc_desc->shapes[i]->mutable_line()->mutable_end(),
                            ray_end);
    ...
    if (enable_collision_detection_) {
        ...
        int numManifolds =
            bt_collision_world->getDispatcher()->getNumManifolds();
        ...
        for (int i = 0; i < numManifolds; i++) {
            ...
            for (int j = 0; j < numContacts; j++) {
                ...
                if (id_to_ent_map_->count(obB->getUserIndex()) > 0) {
                    (*id_to_ent_map_)[obB->getUserIndex()]
                        ->motion()
                        ->set_external_force(-normal_B);
                }
                if (id_to_ent_map_->count(obA->getUserIndex()) > 0) {
                    (*id_to_ent_map_)[obA->getUserIndex()]
                        ->motion()
                        ->set_external_force(normal_B);
                }
            }
            ...
            if (remove_on_collision_ && numContacts > 0) {
                ...
                if (ob_a_is_ent && ob_b_is_ent) {
                    bool same_team =
                        ent_a->id().team_id() == ent_b->id().team_id();
                    if (enable_team_collisions_ && same_team) {
                        // Construct the TeamCollision message
                        auto msg =
                            std::make_shared<Message<sm::TeamCollision>>();
                        ...
                        auto msg =
                            std::make_shared<Message<sm::NonTeamCollision>>();
                        msg->data.set_entity_id_1(ent_a->id().id());
                        msg->data.set_entity_id_2(ent_b->id().id());
                    ...
                    auto msg =
                        std::make_shared<sc::Message<sm::GroundCollision>>();
...
bool BulletCollision::get_ray_tracing(scrimmage::MessageBasePtr request,
    ...
    auto request_cast =
        std::dynamic_pointer_cast<sc::Message<RayTrace::PointCloudWithId>>(
            request);

    if (request_cast == nullptr) {
        std::cout
            << "Could not cast to sc::Message<RayTrace::PointCloudId> request"
            << std::endl;
        return false;
    }
...
std::pair<bool, scrimmage::EntityPtr> BulletCollision::get_entity(
    const int &id) {
...
void BulletCollision::remove_object(const int &id) {
    ...
    if (it_object != objects_.end()) {
        ...
        if (it_object->second.shape != nullptr) {
            ...
            sc::set(it_object->second.shape->mutable_color(),
                    Eigen::Vector3d(0, 0, 0));
