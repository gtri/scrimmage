...
BulletCollision::BulletCollision() {
  ...
  bt_broadphase =
      new bt32BitAxisSweep3(worldAabbMin, worldAabbMax, max_objects_, 0, true);
...
bool BulletCollision::init(std::map<std::string, std::string> &mission_params,
  ...
  remove_on_collision_ =
      sc::get<bool>("remove_on_collision", plugin_params, remove_on_collision_);
  show_collision_shapes_ = sc::get<bool>("show_collision_shapes", plugin_params,
                                         show_collision_shapes_);
  enable_ground_plane_ =
      sc::get<bool>("enable_ground_plane", plugin_params, enable_ground_plane_);
  ...
  enable_non_team_collisions_ = get<bool>(
      "enable_non_team_collisions", plugin_params, enable_non_team_collisions_);
        ...
        if (kv.second->type() == "Ray") {
          ...
          if (rs->automatic_ray_tracing() == true) {
            ...
            if (rs) {
              ...
              // Get the rays (copied internally, so can just assign here),
              // using the model name
              //  so the dirty flag can be checked later.
              pcl.rays = rs->rays();
              for (auto &ray : pcl.rays) {
                Eigen::Vector3d r(rs->max_range(), 0, 0);
                sc::Quaternion rot_vert(Eigen::Vector3d(0, 1, 0),
                                        ray.elevation_rad);
                r = rot_vert.rotate(r);

                sc::Quaternion rot_horiz(Eigen::Vector3d(0, 0, 1),
                                         ray.azimuth_rad);
  ...
  if (enable_ground_plane_) {
    ...
    coll_object->getWorldTransform().setOrigin(
        btVector3((btScalar)0, (btScalar)0, (btScalar)ground_plane_height_));
    bt_collision_world->addCollisionObject(coll_object);
  }
  ...
  if (enable_terrain_) {
    ...
    if (utm_terrain->enable_terrain() == true) {
      ...
      for (vtkIdType index = 0; index < triangularData->GetNumberOfCells();
           index++) {
      ...
      if (t > (pc.max_sample_rate + kv2.second->last_update_time)) {
        ...
        for (RayTrace::PCPoint &pcpoint : pc.points) {
          ...
          if (show_rays_) {
            ...
            sc::set(pc_desc->shapes[i]->mutable_line()->mutable_start(),
                    sensor_pos_w);
    ...
    for (int i = 0; i < numManifolds; i++) {
      ...
      for (int j = 0; j < numContacts; j++) {
        ...
        if (id_to_ent_map_->count(obB->getUserIndex()) > 0) {
          (*id_to_ent_map_)[obB->getUserIndex()]->motion()->set_external_force(
              -normal_B);
        }
        if (id_to_ent_map_->count(obA->getUserIndex()) > 0) {
          (*id_to_ent_map_)[obA->getUserIndex()]->motion()->set_external_force(
              normal_B);
        }
      }
...
bool BulletCollision::get_ray_tracing(scrimmage::MessageBasePtr request,
  ...
  if (request_cast == nullptr) {
    std::cout << "Could not cast to sc::Message<RayTrace::PointCloudId> request"
              << std::endl;
    return false;
  }
  ...
  for (RayTrace::PCPoint &pcpoint : request_cast->data.points) {
    ...
    if (request_cast->data.world_frame == false) {
      ...
      Eigen::Vector4d ray =
          tf_m *
          Eigen::Vector4d(original_ray(0), original_ray(1), original_ray(2), 1);
