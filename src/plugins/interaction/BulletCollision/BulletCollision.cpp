...
REGISTER_PLUGIN(scrimmage::EntityInteraction, scrimmage::interaction::BulletCollision,
...
bool BulletCollision::init(std::map<std::string, std::string>& mission_params,
    ...
    auto ent_gen_cb = [&](scrimmage::MessagePtr<sm::EntityGenerated> msg) {
        ...
        coll_object->getWorldTransform().setOrigin(btVector3(
            (btScalar)ent->state_truth()->pos()(0), (btScalar)ent->state_truth()->pos()(1),
        ...
        if (show_collision_shapes_) {
            objects_[id].shape = sc::shape::make_sphere(ent->state_truth()->pos(), ent->radius(),
                ...
                if (kv.second->type() == "Ray") {
                    ...
                    if (rs->automatic_ray_tracing() == true) {
                        ...
                        if (rs) {
                            ...
                            // Get the rays (copied internally, so can just assign here), using the
                            // model name
    ...
    if (enable_collision_detection_) {
        ...
        for (int i = 0; i < numManifolds; i++) {
            ...
            for (int j = 0; j < numContacts; j++) {
                ...
                Eigen::Vector3d normal_B(pt.m_normalWorldOnB.x(), pt.m_normalWorldOnB.y(),
