...
bool Follow::step_autonomy(double t, double dt) {
    ...
    //           //cout << "Cnt Long: " << it->state().pose().longitude()<<
    //           endl;
    //
    //           double dist;
    //           geod.Inverse(state_.pose().latitude(),
    //           state_.pose().longitude(),
    //                        it->state().pose().latitude(),
    //                        it->state().pose().longitude(), dist);
    //
    //           if (dist < min_dist) {
    //                min_dist = dist;
    //                min_champ = it;
    ...
    //      double x, y, z;
    //      double h = 0;
    //      proj.Forward(min_champ->state().pose().latitude(),
    //      min_champ->state().pose().longitude(), h, x, y, z); double heading =
    //      atan2(x,y) * 180.0 / scrimmage::PI;
