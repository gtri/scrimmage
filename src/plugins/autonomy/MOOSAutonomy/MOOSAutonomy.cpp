...
REGISTER_PLUGIN(scrimmage::Autonomy, scrimmage::autonomy::MOOSAutonomy,
                MOOSAutonomy_plugin)
...
void MOOSAutonomy::init(std::map<std::string, std::string> &params) {
    ...
    moos_script_ = sc::expand_user(
        sc::get<std::string>("moos_script", params, "launch.sh"));
    moos_mission_file_ = sc::expand_user(
        sc::get<std::string>("moos_mission_file", params, "alpha.moos"));
...
bool MOOSAutonomy::step_autonomy(double t, double dt) {
    ...
    for (auto &kv : *contacts_) {
        ...
        moos_node_.PublishNodeReport(
            type, std::to_string(kv.first), "", s->pos()(0), s->pos()(1),
            s->vel().norm(), heading, -s->pos()(2), "kayak", "none", t, "0");
    }
