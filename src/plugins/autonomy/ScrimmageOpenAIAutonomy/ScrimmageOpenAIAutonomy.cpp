...
void ScrimmageOpenAIAutonomy::init(std::map<std::string, std::string> &params) {
  ...
  if (grpc_mode_) {
    ...
    python_cmd_ = std::string("openai_grpc_link.py") + " --port " + port_str +
                  " --actor " + module + ":" + actor_func_str + " --ip " +
                  grpc_address_;
    auto channel =
  ...
  if (nonlearning_mode_) {
    if (not setup_complete_) {
      ...
      auto p = std::dynamic_pointer_cast<ScrimmageOpenAIAutonomy>(
          shared_from_this());
      std::tie(actions_, observations_, actor_func_) = init_actor_func(
          {p}, params_, CombineActors::NO, UseGlobalSensor::NO, grpc_mode_);
      ...
      if (grpc_mode_) {
        ...
        while (attempts++ < 10 && !done) {
          std::cout << "Connecting to gRPC Server attempt: " << attempts
                    << "/10" << std::endl;
          done = send_env();
        }
