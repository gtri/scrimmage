...
void MotorSchemas::init(std::map<std::string, std::string>& params) {
    ...
    for (auto& plugin_override : plugin_overrides_list) {
        auto behavior = make_autonomy<ms::BehaviorBase>(
            plugin_override.name, parent_->plugin_manager(), plugin_override.overrides, parent_,
            state_, id_to_team_map_, id_to_ent_map_, proj_, contacts_, parent_->file_search(),
...
bool MotorSchemas::step_autonomy(double t, double dt) {
    ...
    if (show_shapes_) {
        ...
        if (line_shape_ == nullptr) {
            line_shape_ = sc::shape::make_line(state_->pos(), vel_result + state_->pos(),
