    ...
    for (std::vector<std::string> vec : vecs) {
      ...
      if (servo >= MAX_NUM_SERVOS) {
        ...
        scrimmage::controller::AxisScale at(
            servo, std::stod(vec[2]), std::stod(vec[3]), std::stod(vec[4]),
...
ArduPilot::fdm_packet ArduPilot::state6dof_to_fdm_packet(
  ...
  parent_->projection()->Reverse(state.pos()(0), state.pos()(1), state.pos()(2),
                                 fdm_pkt.latitude, fdm_pkt.longitude,
                                 fdm_pkt.altitude);
  ...
  if (std::isinf(fdm_pkt.airspeed)) {
    std::cout << "ArduPilot: Warning: velocity or wind contains infinite value."
              << std::endl;
    fdm_pkt.airspeed = 0.0;
  }
