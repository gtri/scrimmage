...
bool ArduPilot::step_autonomy(double t, double dt) {
    ...
    if (asynchonous_mode_) {
        ...
        std::size_t nbytes = recv_socket_->receive_from(ba::buffer(recv_buffer_),
                                                        recv_remote_endpoint_, flags, error);
...
ArduPilot::fdm_packet ArduPilot::state6dof_to_fdm_packet(double t,
    ...
    parent_->projection()->Reverse(state.pos()(0), state.pos()(1), state.pos()(2), fdm_pkt.latitude,
