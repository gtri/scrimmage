...
void ROSAirSim::init(std::map<std::string, std::string> &params) {
  ...
  if (ros_cartographer_) {
    cout << "[ROSAirSim] Using transform tree structure for ROS Cartographer."
         << endl;
  }
  cout << " " << endl;
  ...
  auto airsim_image_cb = [&](auto &msg) {
    ...
    if (!img_topic_published) {
      ...
      for (sc::sensor::AirSimImageType a : image_data_) {
        ...
        // Note down camera names since each camera will need its own image
        // transform old_cam_name=true if camera_name already exists
  ...
  if (pub_imu_data_) {
    subscribe<std::vector<sensor::AirSimImuType>>("LocalNetwork", "AirSimImu",
                                                  airsim_imu_cb);
  }
  if (pub_lidar_data_) {
...
bool ROSAirSim::step_autonomy(double t, double dt) {
  ...
  if (pub_imu_data_ && imu_topic_published) {
    ...
    for (sc::sensor::AirSimImuType i : imu_data_) {
      ...
      Eigen::Isometry3f tf_vehicle_imu_ENU =
          get_sensor_pose_from_worldNED_to_vehicleENU(i.vehicle_pose_world_NED,
                                                      i.imu_pose_world_NED);

      // cout << "If not publishing lidar or image data, publish world frame
      // from imu" << endl;
      //////////////////////////////////////////////////////////////////////
  ...
  if (pub_lidar_data_ && lidar_topic_published) {
    ...
    for (sc::sensor::AirSimLidarType l : lidar_data_) {
      ...
      // If not publishing imu, publish world frame from lidar -- 2nd least
      // receiving/ processing time.
      if (!pub_imu_data_) {
        // cout << "If not publishing imu data, publish world frame from lidar"
        // << endl;
        //////////////////////////////////////////////////////////////////////
        // Update and Publish Transforms
        //////////////////////////////////////////////////////////////////////
      ...
      if (show_camera_images_) {
        ...
        if (a.camera_config.pixels_as_float) {
          ...
          if (a.img.channels() == 4) {
            ...
            cout << "Warning: Old AirSim Linux Asset Environments have 4 "
                    "channels. Color images will not display correctly."
                 << endl;
            cout << "Warning: Use Asset Environment versions Linux-v1.3.1+."
                 << endl;
            cv::Mat tempImage;
    ...
    for (std::string cam_name : camera_names_) {
      for (sc::sensor::AirSimImageType a : image_data_) {
        if (a.camera_config.cam_name == cam_name) {
          ...
          // If not publishing lidar or imu data, publish world frame from
          // images
          ...
          // Pose in image_data.pose is in NED, but scrimmage is in ENU so use
          // the conversion done in AirSimSensor Position
          image_trans_.transform.translation.x =
              tf_vehicle_camera_ENU.translation().x();
          image_trans_.transform.translation.y =
              tf_vehicle_camera_ENU.translation().y();
          ...
          break;  // break the image in msg for loop if camera_name is found and
                  // move to next camera_name
        }
      }
    }
  }
