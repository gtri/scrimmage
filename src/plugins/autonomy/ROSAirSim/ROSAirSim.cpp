...
Eigen::Isometry3f ROSAirSim::get_sensor_pose_from_worldNED_to_vehicleENU(
    Eigen::Isometry3f vehicle_pose_world_NED, Eigen::Isometry3f sensor_pose_world_NED) {
    // Get Vehicle Pose from NED to ENU
...
void ROSAirSim::init(std::map<std::string, std::string>& params) {
    ...
    // Create Robot Transform to Base Link -- this allows compatibility for ROS cartographer
    if (ros_cartographer_) {
        ...
        if (!img_topic_published) {
            ...
            for (sc::sensor::AirSimImageType a : image_data_) {
                ...
                cam_info_publishers_.push_back(
                    nh_->advertise<sensor_msgs::CameraInfo>(info_topic_name_pub, 1));
                ...
                // Depth Images (Depth Perspective and Depth Planner) come in as 1 channel float
                // arrays
                ...
                if (!old_cam_name) {
                    ...
                    Eigen::Isometry3f tf_vehicle_camera_ENU =
                        get_sensor_pose_from_worldNED_to_vehicleENU(a.vehicle_pose_world_NED,
        ...
        if (!lidar_topic_published) {
            ...
            for (sc::sensor::AirSimLidarType l : lidar_data_) {
                ...
                lidar_publishers_.push_back(
                    nh_->advertise<sensor_msgs::PointCloud2>(topic_name_pub, 1));
                ...
                if (l.lidar_data.point_cloud.size() > 3) {
                    ...
                    vector<unsigned char> lidar_msg_data(bytes,
                                                         bytes + sizeof(float) * data_std.size());
                ...
                // Note down imu names since each imu will need its own transform
                ...
                Eigen::Isometry3f tf_vehicle_lidar_ENU =
                    get_sensor_pose_from_worldNED_to_vehicleENU(l.vehicle_pose_world_NED,
    ...
    auto airsim_imu_cb = [&](auto& msg) {
        ...
        if (!imu_topic_published) {
            ...
            for (sc::sensor::AirSimImuType i : imu_data_) {
                ...
                // Get pose of lidar in ENU, vehicle frame using Eigen
                Eigen::Isometry3f tf_vehicle_imu_ENU = get_sensor_pose_from_worldNED_to_vehicleENU(
    ...
    if (pub_lidar_data_) {
        subscribe<std::vector<sensor::AirSimLidarType>>("LocalNetwork", "AirSimLidar",
                                                        airsim_lidar_cb);
    }
    if (pub_image_data_) {
        subscribe<std::vector<sensor::AirSimImageType>>("LocalNetwork", "AirSimImages",
                                                        airsim_image_cb);
    }
}
...
bool ROSAirSim::step_autonomy(double t, double dt) {
    ...
    if (pub_imu_data_ && imu_topic_published) {
        ...
        for (sc::sensor::AirSimImuType i : imu_data_) {
            ...
            // cout << "If not publishing lidar or image data, publish world frame from imu" <<
            // endl;
            //////////////////////////////////////////////////////////////////////
            // Update and Publish Transform
            //////////////////////////////////////////////////////////////////////
            // Publish Vehicle Pose using IMU sensor because it has the least receiving/ processing
            // time
            world_trans_.header.stamp = ros::Time::now();
            world_trans_.header.frame_id = "world";
            if (ros_cartographer_) {
    ...
    if (pub_lidar_data_ && lidar_topic_published) {
        ...
        for (sc::sensor::AirSimLidarType l : lidar_data_) {
            ...
            Eigen::Isometry3f tf_world_vehicle_ENU =
                get_vehicle_world_pose_from_NED_to_ENU(l.vehicle_pose_world_NED);
            // Get pose of lidar in ENU, vehicle frame using Eigen
            Eigen::Isometry3f tf_vehicle_lidar_ENU = get_sensor_pose_from_worldNED_to_vehicleENU(
                l.vehicle_pose_world_NED, l.lidar_pose_world_NED);

            // If not publishing imu, publish world frame from lidar -- 2nd least receiving/
            // processing time.
            if (!pub_imu_data_) {
    ...
    if (pub_image_data_ && img_topic_published) {
        ...
        for (sc::sensor::AirSimImageType a : image_data_) {
            ...
            std::string topic_name_pub =
                ros_namespace_ + "/camera/" + camera_name + "/" + image_type_name;
            // std::string info_topic_name_pub = topic_name_pub + "/info";
            topic_name_pub += +"/raw";
            std::string topic_name_match = "/" + topic_name_pub;
            ...
            if (show_camera_images_) {
                ...
                if (a.camera_config.pixels_as_float) {
                    ...
                    // cv::normalize(a.img, tempImage, 0, 1, cv::NORM_MINMAso use if to
                    // determine.X); cout << tempImage << endl;
        ...
        for (std::string cam_name : camera_names_) {
            for (sc::sensor::AirSimImageType a : image_data_) {
                if (a.camera_config.cam_name == cam_name) {
                    ...
                    // Pose in image_data.pose is in NED, but scrimmage is in ENU so use the
                    // conversion done in AirSimSensor Position
                    image_trans_.transform.translation.x = tf_vehicle_camera_ENU.translation().x();
                    image_trans_.transform.translation.y = tf_vehicle_camera_ENU.translation().y();
                    image_trans_.transform.translation.z = tf_vehicle_camera_ENU.translation().z();
                    // Orientation
                    ...
                    break;  // break the image in msg for loop if camera_name is found and move to
                            // next camera_name
                }
            }
        }
