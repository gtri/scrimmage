...
REGISTER_PLUGIN(scrimmage::Autonomy, scrimmage::autonomy::motor_schemas::AvoidEntityMS,
...
AvoidEntityMS::AvoidEntityMS()
    : sphere_of_influence_(10.0), minimum_range_(5.0), avoid_non_team_(true) {
}
...
bool AvoidEntityMS::step_autonomy(double t, double dt) {
    ...
    if (show_shapes_) {
        ...
        if (circle_shape_ == nullptr) {
            circle_shape_ =
                sc::shape::make_circle(state_->pos(), state_->quat(), sphere_of_influence_,
        ...
        if (line_shape_ == nullptr) {
            line_shape_ = sc::shape::make_line(state_->pos(), desired_vector_ + state_->pos(),
