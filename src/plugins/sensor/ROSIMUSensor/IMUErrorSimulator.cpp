...
void IMUErrorSimulator::CalculateParametersForFixedDeltaT(IMUErrorBudgetTemplate& errorBudget) {
    ...
    if (InRunBiasOption == false) {
        ...
        AccelBiasInRunStdDev.x() = errorBudget.XAccelAxisInRunBiasMPS2StdDev *
                                   sqrt(1 - exp(-2 * NominalDeltaT / errorBudget.XAccelAxisInRunBiasTimeConstantSec));
        AccelBiasInRunStdDev.y() = errorBudget.YAccelAxisInRunBiasMPS2StdDev *
                                   sqrt(1 - exp(-2 * NominalDeltaT / errorBudget.YAccelAxisInRunBiasTimeConstantSec));
        AccelBiasInRunStdDev.z() = errorBudget.ZAccelAxisInRunBiasMPS2StdDev *
        ...
        AccelBiasInRun =
            Eigen::Vector3d(errorBudget.XAccelAxisInRunBiasMPS2StdDev, errorBudget.YAccelAxisInRunBiasMPS2StdDev,
    ...
    AccelVRWMSStdDev = sqrt(NominalDeltaT) * Eigen::Vector3d(errorBudget.XAccelVRWMPS2PerRtHzStdDev,
    ...
    if (InRunBiasOption == false) {
        ...
        GyroBiasInRunStdDev.x() = errorBudget.XGyroAxisInRunBiasRadPerSecStdDev *
                                  sqrt(1 - exp(-2 * NominalDeltaT / errorBudget.XGyroAxisInRunBiasTimeConstantSec));
        GyroBiasInRunStdDev.y() = errorBudget.YGyroAxisInRunBiasRadPerSecStdDev *
                                  sqrt(1 - exp(-2 * NominalDeltaT / errorBudget.YGyroAxisInRunBiasTimeConstantSec));
        GyroBiasInRunStdDev.z() = errorBudget.ZGyroAxisInRunBiasRadPerSecStdDev *
    ...
    GyroARWRadStdDev = sqrt(NominalDeltaT) * Eigen::Vector3d(errorBudget.XGyroARWRadPerSecPerRtHzStdDev,
...
NoisyIMUData IMUErrorSimulator::EachCycle(IMUErrorBudgetTemplate& errorBudget,
    ...
    OutputBodyFrameAccelErrors = TotalAccelBias + errorBudget.SampleFrequency * AccelVRWMSDraws +
    ...
    quantizedAccelOutput = errorBudget.AccelDeltaVQuantizationMPS *
    ...
    OutputBodyFrameAccelErrors =
        errorBudget.SampleFrequency * (quantizedAccelOutput - InputDeltaVBodyWRTInertialInBody).array();
    ...
    OutputBodyFrameRateGyroErrors = TotalGyroBias.array() + errorBudget.SampleFrequency * GyroARWRadDraws.array() +
    ...
    Eigen::Vector3d OutputNoisyDeltaThetaBodyWRTInertialInBody =
        InputDeltaThetaBodyWRTInertialInBody.array() + NominalDeltaT * OutputBodyFrameRateGyroErrors.array();
    ...
    GyroQuantizationResiduals =
        OutputNoisyDeltaThetaBodyWRTInertialInBody + GyroQuantizationResiduals - quantizedGyroOutput;

    // Calculate totality of errors added to gyro data:
    OutputBodyFrameRateGyroErrors =
        errorBudget.SampleFrequency * (quantizedGyroOutput - InputDeltaThetaBodyWRTInertialInBody).array();
