...
void IMUErrorSimulator::CalculateParametersForFixedDeltaT(
  ...
  if (InRunBiasOption == false) {
    AccelBiasPhi.x() =
        exp(-NominalDeltaT / errorBudget.XAccelAxisInRunBiasTimeConstantSec);
    AccelBiasPhi.y() =
        exp(-NominalDeltaT / errorBudget.YAccelAxisInRunBiasTimeConstantSec);
    AccelBiasPhi.z() =
        exp(-NominalDeltaT / errorBudget.ZAccelAxisInRunBiasTimeConstantSec);
...
NoisyIMUData IMUErrorSimulator::EachCycle(
  ...
  GyroARWRadDraws =
      GyroARWRadStdDev.array() * RandomVector(GyroARWDist, GyroARWRNG).array();
  ...
  quantizedGyroOutput =
      ...
      ((OutputNoisyDeltaThetaBodyWRTInertialInBody + GyroQuantizationResiduals)
           .array() /
       errorBudget.GyroDeltaThetaQuantizationRadians)
          .round();
  // Calculate residuals from quantization process and later add them to
