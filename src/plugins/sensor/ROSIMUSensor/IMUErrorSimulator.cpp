 ...
 * @section DESCRIPTION
 * Class for adding realistic error/noise to 'perfect' IMU data generated from
...
Eigen::Vector3d IMUErrorSimulator::RandomVector(
    std::normal_distribution<double>& distribution, std::mt19937& generator) {
    Eigen::Vector3d newVec;
    newVec << distribution(generator), distribution(generator),
        distribution(generator);
    return newVec;
}

void IMUErrorSimulator::PerformInitialRandomDraws(
    ...
    AccelBiasTurnOn.x() =
        errorBudget.XAccelAxisTurnOnBiasMPS2StdDev * InitDist(InitRNG);
    AccelBiasTurnOn.y() =
        errorBudget.YAccelAxisTurnOnBiasMPS2StdDev * InitDist(InitRNG);
    AccelBiasTurnOn.z() =
        errorBudget.ZAccelAxisTurnOnBiasMPS2StdDev * InitDist(InitRNG);

    // Gyro Error Terms
    // Get random draws for gyro scale factor errors:
    GyroScaleFactorErr.x() =
        (1e-6) * errorBudget.XGyroAxisScaleFactorPPMStdDev * InitDist(InitRNG);
    GyroScaleFactorErr.y() =
        (1e-6) * errorBudget.YGyroAxisScaleFactorPPMStdDev * InitDist(InitRNG);
    GyroScaleFactorErr.z() =
        (1e-6) * errorBudget.ZGyroAxisScaleFactorPPMStdDev * InitDist(InitRNG);
    // Place gyro scale factor errors in matrix Mg:
    Mg = GyroScaleFactorErr.asDiagonal();

    // Get random draws for gyro turn-on biases
    GyroBiasTurnOn.x() =
        errorBudget.XGyroAxisTurnOnBiasRadPerSecStdDev * InitDist(InitRNG);
    GyroBiasTurnOn.y() =
        errorBudget.YGyroAxisTurnOnBiasRadPerSecStdDev * InitDist(InitRNG);
    GyroBiasTurnOn.z() =
        errorBudget.ZGyroAxisTurnOnBiasRadPerSecStdDev * InitDist(InitRNG);
}

void IMUErrorSimulator::CalculateParametersForFixedDeltaT(
    IMUErrorBudgetTemplate& errorBudget) {
    ...
    if (InRunBiasOption == false) {
        AccelBiasPhi.x() = exp(-NominalDeltaT /
                               errorBudget.XAccelAxisInRunBiasTimeConstantSec);
        AccelBiasPhi.y() = exp(-NominalDeltaT /
                               errorBudget.YAccelAxisInRunBiasTimeConstantSec);
        AccelBiasPhi.z() = exp(-NominalDeltaT /
                               errorBudget.ZAccelAxisInRunBiasTimeConstantSec);

        AccelBiasInRunStdDev.x() =
            errorBudget.XAccelAxisInRunBiasMPS2StdDev *
            sqrt(1 - exp(-2 * NominalDeltaT /
                         errorBudget.XAccelAxisInRunBiasTimeConstantSec));
        ...
        AccelBiasInRun =
            Eigen::Vector3d(errorBudget.XAccelAxisInRunBiasMPS2StdDev,
                            errorBudget.YAccelAxisInRunBiasMPS2StdDev,
                            errorBudget.ZAccelAxisInRunBiasMPS2StdDev)
                .asDiagonal() *
    ...
    if (InRunBiasOption == false) {
        GyroBiasPhi.x() =
            exp(-NominalDeltaT / errorBudget.XGyroAxisInRunBiasTimeConstantSec);
        GyroBiasPhi.y() =
            exp(-NominalDeltaT / errorBudget.YGyroAxisInRunBiasTimeConstantSec);
        GyroBiasPhi.z() =
            exp(-NominalDeltaT / errorBudget.ZGyroAxisInRunBiasTimeConstantSec);

        GyroBiasInRunStdDev.x() =
            errorBudget.XGyroAxisInRunBiasRadPerSecStdDev *
            sqrt(1 - exp(-2 * NominalDeltaT /
                         errorBudget.XGyroAxisInRunBiasTimeConstantSec));
        GyroBiasInRunStdDev.y() =
            errorBudget.YGyroAxisInRunBiasRadPerSecStdDev *
            sqrt(1 - exp(-2 * NominalDeltaT /
                         errorBudget.YGyroAxisInRunBiasTimeConstantSec));
        GyroBiasInRunStdDev.z() =
            errorBudget.ZGyroAxisInRunBiasRadPerSecStdDev *
            sqrt(1 - exp(-2 * NominalDeltaT /
                         errorBudget.ZGyroAxisInRunBiasTimeConstantSec));
        // do initial draw for in-run bias
        //                GyroBiasInRun =
        ...
        GyroBiasInRun =
            Eigen::Vector3d(errorBudget.XGyroAxisInRunBiasRadPerSecStdDev,
                            errorBudget.YGyroAxisInRunBiasRadPerSecStdDev,
                            errorBudget.ZGyroAxisInRunBiasRadPerSecStdDev)
                .asDiagonal() *
...
NoisyIMUData IMUErrorSimulator::EachCycle(
    IMUErrorBudgetTemplate& errorBudget,
    Eigen::Vector3d InputDeltaVBodyWRTInertialInBody,
    Eigen::Vector3d InputDeltaThetaBodyWRTInertialInBody) {
    Eigen::Vector3d quantizedAccelOutput, OutputBodyFrameAccelErrors,
        quantizedGyroOutput, OutputBodyFrameRateGyroErrors;
    NoisyIMUData returnValue;

    if (!InRunBiasOption) {
        TotalAccelBias = AccelBiasTurnOn + AccelBiasInRun;
        AccelBiasInRun = AccelBiasPhi.array() * AccelBiasInRun.array() +
                         AccelBiasInRunStdDev.array() *
                             RandomVector(AccelBiasDist, AccelBiasRNG).array();
    }

    AccelVRWMSDraws = AccelVRWMSStdDev.array() *
                      RandomVector(AccelVRWDist, AccelVRWRNG).array();
    ...
    Eigen::Vector3d OutputNoisyDeltaVBodyWRTInertialInBody =
        InputDeltaVBodyWRTInertialInBody +
        NominalDeltaT * OutputBodyFrameAccelErrors;
    ...
    quantizedAccelOutput =
        ...
        ((OutputNoisyDeltaVBodyWRTInertialInBody + AccelQuantizationResiduals)
             .array() /
         errorBudget.AccelDeltaVQuantizationMPS)
            .round();
    // Calculate residuals from quantization process and later add them to
    // the next accel measurements to be quantized
    AccelQuantizationResiduals = OutputNoisyDeltaVBodyWRTInertialInBody +
    ...
    if (InRunBiasOption == false) {
        ...
        GyroBiasInRun = GyroBiasPhi.array() * GyroBiasInRun.array() +
                        GyroBiasInRunStdDev.array() *
                            RandomVector(GyroBiasDist, GyroBiasRNG).array();
    }
    // do random draw for gyro angel random walk:
    GyroARWRadDraws = GyroARWRadStdDev.array() *
                      RandomVector(GyroARWDist, GyroARWRNG).array();

    // Calculate errors to be added to the gyro data
    OutputBodyFrameRateGyroErrors =
        TotalGyroBias.array() +
        errorBudget.SampleFrequency * GyroARWRadDraws.array() +
        errorBudget.SampleFrequency *
            (Mg * InputDeltaThetaBodyWRTInertialInBody).array();
    ...
    quantizedGyroOutput = errorBudget.GyroDeltaThetaQuantizationRadians *
                          ((OutputNoisyDeltaThetaBodyWRTInertialInBody +
                            GyroQuantizationResiduals)
                               .array() /
