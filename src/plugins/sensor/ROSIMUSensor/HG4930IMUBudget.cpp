...
void HG4930IMUBudget::CalculateParameters() {
  ...
  XAccelAxisInRunBiasContinuousProcessNoiseVar =
      ...
      XAccelAxisInRunBiasTimeConstantSec;  // variance of continuous-time
                                           // process noise driving in-run bias
  ...
  XAccelVRWMPS2PerRtHzStdDev =
      (9.8 * AccelVRWGPerRtHz);  // velocity random walk (VRW) 1-sigma in units
                                 // of meters per second (MPS) squared
  ...
  XGyroAxisInRunBiasTimeConstantSec =
      GyroBiasInstabilityTimeConstantSec;  // in-run bias time constant in units
                                           // of seconds
