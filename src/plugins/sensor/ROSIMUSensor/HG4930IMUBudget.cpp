...
void HG4930IMUBudget::CalculateParameters() {
    ...
    XAccelAxisInRunBiasTimeConstantSec =
        AccelBiasTimeConstantSec;  // in-run bias time constant in units of seconds
    ...
    XAccelAxisInRunBiasContinuousProcessNoiseVar =
        ...
        XAccelAxisInRunBiasTimeConstantSec;  // variance of continuous-time process noise driving
                                             // in-run bias
    YAccelAxisInRunBiasContinuousProcessNoiseVar =
        (2 * pow(YAccelAxisInRunBiasMPS2StdDev, 2)) /
        YAccelAxisInRunBiasTimeConstantSec;  // variance of continuous-time process noise driving
                                             // in-run bias
    ZAccelAxisInRunBiasContinuousProcessNoiseVar =
        (2 * pow(ZAccelAxisInRunBiasMPS2StdDev, 2)) /
        ZAccelAxisInRunBiasTimeConstantSec;  // variance of continuous-time process noise driving
                                             // in-run bias
    // Velocity random walk
    XAccelVRWMPS2PerRtHzStdDev =
        (9.8 * AccelVRWGPerRtHz);  // velocity random walk (VRW) 1-sigma in units of meters per
                                   // second (MPS) squared
    ...
    // Turn-on Bias
    double GyroBiasStdDevRadPerSec = GyroBiasRepeatabilityDegPerHr * (M_PI / 180.0) *
    ...
    ZGyroAxisTurnOnBiasRadPerSecStdDev =
        GyroBiasStdDevRadPerSec;  // 1-sigma in units of radians/second
    // In-run Bias
    GyroBiasStdDevRadPerSec = GyroBiasInstabilityDegPerHr * (M_PI / 180.0) *
                              (1.0 / 3600.0);  // convert to 1-sigma in radians/sec
    // Now load into appropriate variables
    ...
    XGyroAxisInRunBiasContinuousProcessNoiseVar =
        ...
        GyroBiasInstabilityTimeConstantSec;  // variance of continuous-time process noise driving
                                             // in-run bias
    ...
    XGyroAxisInRunBiasTimeConstantSec =
        GyroBiasInstabilityTimeConstantSec;  // in-run bias time constant in units of seconds
