    ...
    if (t_1.length() > 0) {
      ...
      if ((tokens_2.size() == 6) && (tokens_2[0] == vehicle_name_)) {
        try {
          ...
          cout << "Parse Error: Check camera_config's in AirSimSensor.xml"
               << endl;
        }  // end try, catch
      }
...
void AirSimSensor::init(std::map<std::string, std::string> &params) {
  ...
  if (save_airsim_data_) {
    cout << "[AirSimSensor] Saving camera images and airsim_data.csv of pose "
...
void AirSimSensor::request_images() {
  ...
  for (int i = 0; i < 11; i++) {
    if (img_client->getConnectionState() !=
      ...
      if (i == 10) {
        cout << "\n[AirSimSensor] Warning: Image client could not connect to "
                "AirSim."
             << endl;
  ...
  while (running) {
    ...
    for (CameraConfig c : cam_configs_) {
      // Depth Images (Depth Perspective and Depth Planner) come in as 1 channel
      // float arrays
    ...
    // If response vector contains new data, create img_msg, transfer data into
    // message and publish cout << response_vector.size() << endl;
    if (response_vector.size() > 0) {
      ...
      for (ImageResponse response : response_vector) {
        ...
        if (a.camera_config.pixels_as_float) {
          ...
          a.img =
              cv::Mat(a.camera_config.height, a.camera_config.width, CV_32FC1);
          ...
          if ((static_cast<int>(response.image_data_uint8.size())) >
            ...
            a.img =
                cv::Mat(a.camera_config.height, a.camera_config.width, CV_8UC4);
            memcpy(a.img.data, response.image_data_uint8.data(),
                   response.image_data_uint8.size() * sizeof(uint8_t));
          } else {
            // image has 3 channels
            a.img =
                cv::Mat(a.camera_config.height, a.camera_config.width, CV_8UC3);
            memcpy(a.img.data, response.image_data_uint8.data(),
                   response.image_data_uint8.size() * sizeof(uint8_t));
          }
    ...
    // t_start).count(); cout << "avg image period: " << image_period_ << "s" <<
    // endl;
  }  // end while running
...
void AirSimSensor::request_lidar() {
  ...
  for (int i = 0; i < 11; i++) {
    if (lidar_client->getConnectionState() !=
      ...
      // If we haven't been able to connect to AirSim, warn the user and return
      if (i == 10) {
        cout << "\n[AirSimSensor] Warning: LIDAR client could not connect to "
                "AirSim."
             << endl;
        cout << "[AirSimSensor] Warning: Please start Unreal/AirSim before the "
                "SCRIMMAGE mission.\n"
  ...
  while (running) {
    ...
    // std::chrono::duration_cast<std::chrono::duration<double>>(t_end -
    // t_start).count(); cout << "avg lidar period: " << lidar_period_ << "s" <<
    // endl;
  }
}
...
void AirSimSensor::request_imu() {
  ...
  for (int i = 0; i < 11; i++) {
    if (imu_client->getConnectionState() !=
      ...
      // If we haven't been able to connect to AirSim, warn the user and return
      if (i == 10) {
        cout << "\n[AirSimSensor] Warning: IMU client could not connect to "
                "AirSim."
             << endl;
        cout << "[AirSimSensor] Warning: Please start Unreal/AirSim before the "
                "SCRIMMAGE mission.\n"
  ...
  while (running) {
    ...
    for (const auto &imu_name : imu_names_) {
      ...
      // Get pose of IMU
      // AirLib API does not give IMU position so use position for center of
      // vehicle
...
bool AirSimSensor::step() {
  ...
  if (!client_connected_) {
    ...
    for (int i = 0; i < 11; i++) {
      if (sim_client_->getConnectionState() !=
        ...
        // return.
        if (i == 10) {
          cout << "\n[AirSimSensor] Warning: Sim client could not connect to "
                  "AirSim."
               << endl;
          cout << "[AirSimSensor] Warning: Please start Unreal/AirSim before "
  ...
  // pitch, roll, yaw
  // note, the negative pitch and yaw are required because of the wsu coordinate
  // frame
  ...
  if (get_image_data_) {
    ...
    // auto im_msg_step =
    // std::make_shared<sc::Message<std::vector<AirSimImageType>>>(); // NOLINT
  ...
  if (get_lidar_data_) {
    ...
    // auto lidar_msg_step =
    // std::make_shared<sc::Message<std::vector<AirSimLidarType>>>(); // NOLINT
    ...
    lidar_msg_mutex_.lock();
    std::shared_ptr<sc::Message<std::vector<AirSimLidarType>>> lidar_msg_step =
        lidar_msg_;
  ...
  if (get_imu_data_) {
    // auto imu_msg_step =
    // std::make_shared<sc::Message<std::vector<AirSimImuType>>>(); // NOLINT
