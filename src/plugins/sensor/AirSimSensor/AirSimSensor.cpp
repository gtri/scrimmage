    ...
    while (running) {
        ...
        if (response_vector.size() > 0) {
            ...
            for (ImageResponse response : response_vector) {
                ...
                Eigen::Isometry3f tf_world_camera_NED(
                    translation_trans_camera * rotation_quat_camera);
        ...
        t_end = std::chrono::high_resolution_clock::now();
        // double image_period_ = std::chrono::duration_cast<std::chrono::duration<double>>(t_end -
        // t_start).count(); cout << "avg image period: " << image_period_ << "s" << endl;
    }  // end while running
...
void AirSimSensor::request_lidar() {
    ...
    while (running) {
        ...
        t_end = std::chrono::high_resolution_clock::now();
        // double lidar_period_ = std::chrono::duration_cast<std::chrono::duration<double>>(t_end -
        // t_start).count(); cout << "avg lidar period: " << lidar_period_ << "s" << endl;
    }
}
...
void AirSimSensor::request_imu() {
    ...
    while (running) {
        ...
        t_end = std::chrono::high_resolution_clock::now();
        // double imu_period_ = std::chrono::duration_cast<std::chrono::duration<double>>(t_end -
        // t_start).count(); cout << "avg imu period: " << imu_period_ << "s" << endl;
    }
}
...
bool AirSimSensor::save_data(
    MessagePtr<std::vector<AirSimImageType>>& im_msg,
    sc::StatePtr& state,
    int frame_num) {
    // Get timestamp
