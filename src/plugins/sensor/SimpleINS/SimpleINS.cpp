...
REGISTER_PLUGIN(scrimmage::Sensor, scrimmage::sensor::SimpleINS,
                SimpleINS_plugin)
...
bool SimpleINS::step() {
    ...
    if (init_m_) {
        m_ = Eigen::MatrixXd::Identity(ns.covariance().rows(),
                                       ns.covariance().cols());
        init_m_ = false;
    }
    ...
    if (!gps_fix_) {
        ...
        // GPS fix on - wait on surface timer and snap back to an identity
        // matrix
        m_(0, 0) += (pos_noise_0 < 0 ? -1 * pos_noise_0 : pos_noise_0);
        m_(1, 1) += (pos_noise_1 < 0 ? -1 * pos_noise_1 : pos_noise_1);
        // GPS fix on - wait on surface timer and snap back to an identity
    ...
    for (int i = 0; i < 3; i++) {
        // std::cout << "Accelration for " << i << ", is: " << (accel_(i)) <<
        // std::endl;
