...
void ContactBlobCamera::init(std::map<std::string, std::string>& params) {
    std::stringstream ss;
    ss << parent_->id().id();
    parameters_file_.open(
        parent_->mp()->log_dir() + "/blob_sensor_parameters_" + ss.str() + ".txt");
    ...
    if (log_detections_) {
        detections_file_.open(
            parent_->mp()->log_dir() + "/blob_sensor_detections_" + std::to_string(camera_id_)
            + ".txt");
    }
...
void ContactBlobCamera::draw_frustum(
    const std::vector<scrimmage_proto::ShapePtr>& frustum_shapes,
    double x_rot,
    double y_rot,
    double z_rot) {
...
void ContactBlobCamera::contacts_to_bounding_boxes(
    ...
    for (auto& kv : contacts) {
        ...
        if ((kv.second.state()->pos_const() - sensor_frame.pos_const()).norm()
            > max_detect_range_) {
            continue;
        }
        ...
        Eigen::Vector3d rel_pos = sensor_frame.rel_pos_local_frame(kv.second.state()->pos_const());
...
Eigen::Vector2d ContactBlobCamera::project_rel_3d_to_2d(Eigen::Vector3d rel_pos) {
    ...
    Eigen::Vector2d ndc(
        (s(0) + canvas_width_ / 2.0) / canvas_width_,
