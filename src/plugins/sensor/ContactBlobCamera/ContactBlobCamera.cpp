...
REGISTER_PLUGIN(scrimmage::Sensor, scrimmage::sensor::ContactBlobCamera,
                ContactBlobCamera_plugin)
...
void ContactBlobCamera::init(std::map<std::string, std::string> &params) {
    std::stringstream ss;
    ss << parent_->id().id();
    parameters_file_.open(parent_->mp()->log_dir() +
                          "/blob_sensor_parameters_" + ss.str() + ".txt");

    gener_ = parent_->random()->gener();

    window_name_ = sc::get<std::string>("window_name", params, window_name_);
    ignore_real_entities_ =
        sc::get<bool>("ignore_real_entities", params, ignore_real_entities_);
    ...
    std::string sim_det_str =
        sc::get<std::string>("simulated_detections", params, "");
    ...
    plugin_params_["max_detect_range"] =
        sc::get<double>("max_detect_range", params, 1000);
    plugin_params_["focal_length"] =
        sc::get<double>("focal_length", params, 1.0);
    plugin_params_["fps"] = sc::get<double>("frames_per_second", params, 10);
    plugin_params_["az_thresh"] =
        sc::Angles::deg2rad(sc::get<double>("azimuth_fov", params, 360));
    plugin_params_["el_thresh"] =
        sc::Angles::deg2rad(sc::get<double>("elevation_fov", params, 360));
    plugin_params_["fn_prob"] =
        sc::get<double>("false_negative_probability", params, 0.1);
    plugin_params_["fp_prob"] =
        sc::get<double>("false_positive_probability", params, 0.1);
    plugin_params_["max_false_positives"] =
        sc::get<int>("max_false_positives_per_frame", params, 10);
    ...
    if (log_detections_) {
        detections_file_.open(parent_->mp()->log_dir() +
                              "/blob_sensor_detections_" +
                              std::to_string(camera_id_) + ".txt");
    }
     ...
     */
    auto params_cb = [&](sc::MessagePtr<std::map<std::string, double>> msg) {
        set_plugin_params(msg->data);
    };
    subscribe<std::map<std::string, double>>("LocalNetwork", "BlobPluginParams",
                                             params_cb);
...
void ContactBlobCamera::draw_frustum(
    const std::vector<scrimmage_proto::ShapePtr> &frustum_shapes, double x_rot,
    double y_rot, double z_rot) {
    if (frustum_shapes.size() != 8) {
        std::cerr
            << "ContactBlobCamera::draw_frustum: ERROR: input shape vector "
               "must be size 8"
            << std::endl;
        return;
    }
    double sensor_footprint_height =
        max_detect_range_ * tan(el_thresh_ / 2) * 2;
    double sensor_footprint_width = max_detect_range_ * tan(az_thresh_ / 2) * 2;
    Eigen::Vector3d sensor_UL(max_detect_range_, sensor_footprint_width / 2,
                              -sensor_footprint_height / 2);
    Eigen::Vector3d sensor_UR(max_detect_range_, sensor_footprint_width / 2,
                              sensor_footprint_height / 2);
    Eigen::Vector3d sensor_LR(max_detect_range_, -sensor_footprint_width / 2,
                              sensor_footprint_height / 2);
    Eigen::Vector3d sensor_LL(max_detect_range_, -sensor_footprint_width / 2,
                              -sensor_footprint_height / 2);
    std::vector<Eigen::Vector3d> sensor_scene_bb = {
        sensor_UL, sensor_UR, sensor_LR, sensor_LL, sensor_UL};
    ...
    auto A = Eigen::Translation3d(parent_->state_truth()->pos()) * z_rotation *
    ...
    for (int idx_bb = 0; idx_bb != 4; ++idx_bb) {
        ...
        scrimmage::set(
            frustum_shapes.at(idx_bb)->mutable_line()->mutable_start(),
            box_edge_start);
        scrimmage::set(frustum_shapes.at(idx_bb)->mutable_line()->mutable_end(),
                       box_edge_end);

        // set fov line
        scrimmage::set(
            frustum_shapes.at(idx_fov_edge)->mutable_line()->mutable_start(),
            fov_edge_start);
        scrimmage::set(
            frustum_shapes.at(idx_fov_edge)->mutable_line()->mutable_end(),
            box_edge_end);
    }
    // draw all lines
...
void ContactBlobCamera::contacts_to_bounding_boxes(
    const scrimmage::State &sensor_frame, scrimmage::ContactMap &contacts,
    ...
    for (auto &kv : contacts) {
        ...
        Eigen::Vector3d rel_pos =
            sensor_frame.rel_pos_local_frame(kv.second.state()->pos());
        ...
        Eigen::Vector3d p1(rel_pos(0) + v_r_1(0), rel_pos(1) + v_r_1(1),
                           rel_pos(2));

        Eigen::Vector3d p2(rel_pos(0) + v_r_2(0), rel_pos(1) + v_r_2(1),
                           rel_pos(2));
        ...
        double object_img_half_width =
            std::max(1.0, object_img_radius + (*size_noise_[0])(*gener_));
        double object_img_half_height =
            std::max(1.0, object_img_radius + (*size_noise_[1])(*gener_));
        ...
        cv::Rect rect(
            std::floor(raster_center(0)) - std::floor(object_img_half_width),
...
void ContactBlobCamera::add_false_positives(
    ...
    for (int i = 0; i < max_false_positives_; i++) {
        ...
        // Add bounding box to frame around object
        cv::Rect rect(
            std::floor(raster_center(0)) - std::floor(object_img_radius),
...
bool ContactBlobCamera::step() {
    ...
    sensor_frame.quat() = static_cast<sc::Quaternion>(
        parent_->state_truth()->quat() * this->transform()->quat());
    sensor_frame.pos() =
        this->transform()->pos() + parent_->state_truth()->pos();

    if (show_frustum_) {
        draw_frustum(frustum_shapes_, sensor_frame.quat().roll(),
                     sensor_frame.quat().pitch(), sensor_frame.quat().yaw());
    }
    ...
    if (show_sim_contacts_ &&
        ...
        for (auto &kv : sim_contacts_) {
            ...
            sc::set(sim_tgt_sphere_->mutable_sphere()->mutable_center(),
                    sphere_center);
...
Eigen::Vector2d ContactBlobCamera::project_rel_3d_to_2d(
    Eigen::Vector3d rel_pos) {
    ...
    Eigen::Vector2d r(ndc(0) * img_width_, (1 - ndc(1)) * img_height_);
    return r;
}
...
bool ContactBlobCamera::in_field_of_view(Eigen::Vector3d rel_pos) {
    ...
    if (std::abs(az) > (az_thresh_ / 2) || std::abs(el) > (el_thresh_ / 2)) {
        return false;
    }
    return true;
}
...
void ContactBlobCamera::draw_object_with_bounding_box(
    ...
    Eigen::Vector2i c(std::floor(center(0)), std::floor(center(1)));

    cv::circle(frame, cv::Point(c(0), c(1)), std::floor(radius),
...
void ContactBlobCamera::set_plugin_params(
    std::map<std::string, double> params) {
    if (params["senderId"] != parent_->id().id()) {
        return;
    }
    ...
    if (std_dev_w_ > img_width_) {
        std_dev_w_ = 10;
    }  // TODO: use standard deviation values to inform the false positives
    if (std_dev_h_ > img_height_) {
        std_dev_h_ = 10;
    }
