...
void ContactBlobCamera::draw_frustum(const std::vector<scrimmage_proto::ShapePtr> &frustum_shapes,
    ...
    Eigen::Vector3d sensor_UL(
        max_detect_range_, sensor_footprint_width / 2, -sensor_footprint_height / 2);
    Eigen::Vector3d sensor_UR(
        max_detect_range_, sensor_footprint_width / 2, sensor_footprint_height / 2);
    Eigen::Vector3d sensor_LR(
        max_detect_range_, -sensor_footprint_width / 2, sensor_footprint_height / 2);
    Eigen::Vector3d sensor_LL(
        max_detect_range_, -sensor_footprint_width / 2, -sensor_footprint_height / 2);
    ...
    sc::path_to_lines(
        sensor_scene_bb, scene_bb_line, std::static_pointer_cast<EntityPlugin>(shared_from_this()));
...
void ContactBlobCamera::contacts_to_bounding_boxes(
    ...
    for (auto &kv : contacts) {
        ...
        if (object_img_radius > 0) {
            draw_object_with_bounding_box(
                msg->data.frame, kv.second.id().id(), rect, raster_center, object_img_radius);
        }
...
void ContactBlobCamera::add_false_positives(
    ...
    for (int i = 0; i < max_false_positives_; i++) {
        ...
        if (object_img_radius > 0) {
            draw_object_with_bounding_box(
                msg->data.frame, id, rect, raster_center, object_img_radius);
        }
...
bool ContactBlobCamera::step() {
    ...
    if (show_frustum_) {
        draw_frustum(frustum_shapes_,
                     sensor_frame.quat().roll(),
                     sensor_frame.quat().pitch(),
...
void ContactBlobCamera::draw_object_with_bounding_box(cv::Mat &frame,
                                                      const int &id,
                                                      const cv::Rect &rect,
    ...
    cv::circle(
        frame, cv::Point(c(0), c(1)), std::floor(radius), cv::Scalar(255, 255, 255), -1, 8, 0);
