...
void ContactBlobCamera::init(std::map<std::string, std::string>& params) {
    std::stringstream ss;
    ss << parent_->id().id();
    parameters_file_.open(parent_->mp()->log_dir() + "/blob_sensor_parameters_" + ss.str()
                          + ".txt");
    ...
    if (get_vec_of_vecs(sim_det_str, vecs, ", ")) {
        ...
        for (auto& vec : vecs) {
            ...
            sc::Quaternion quat(std::stod(vec[6]), std::stod(vec[7]), std::stod(vec[8]));

            sc::StatePtr cnt_state = std::make_shared<sc::State>(
                Eigen::Vector3d(x, y, z), Eigen::Vector3d(0, 0, 0), Eigen::Vector3d(0, 0, 0), quat);
    ...
    if (log_detections_) {
        detections_file_.open(parent_->mp()->log_dir() + "/blob_sensor_detections_"
...
void ContactBlobCamera::draw_frustum(const std::vector<scrimmage_proto::ShapePtr>& frustum_shapes,
    ...
    Eigen::Vector3d sensor_LL(max_detect_range_, -sensor_footprint_width / 2,
                              -sensor_footprint_height / 2);
    std::vector<Eigen::Vector3d> sensor_scene_bb = {sensor_UL, sensor_UR, sensor_LR, sensor_LL,
                                                    sensor_UL};
    ...
    auto A =
        Eigen::Translation3d(parent_->state_truth()->pos()) * z_rotation * y_rotation * x_rotation;
    ...
    for (int idx_bb = 0; idx_bb != 4; ++idx_bb) {
        ...
        // set fov line
        scrimmage::set(frustum_shapes.at(idx_fov_edge)->mutable_line()->mutable_start(),
                       fov_edge_start);
        scrimmage::set(frustum_shapes.at(idx_fov_edge)->mutable_line()->mutable_end(),
                       box_edge_end);
    }
    // draw all lines
...
void ContactBlobCamera::contacts_to_bounding_boxes(
    ...
    for (auto& kv : contacts) {
        ...
        if ((kv.second.state()->pos() - sensor_frame.pos()).norm() > max_detect_range_) {
            continue;
        }
        ...
        Eigen::Vector3d p1(rel_pos(0) + v_r_1(0), rel_pos(1) + v_r_1(1), rel_pos(2));

        Eigen::Vector3d p2(rel_pos(0) + v_r_2(0), rel_pos(1) + v_r_2(1), rel_pos(2));
        ...
        if (object_img_radius > 0) {
            draw_object_with_bounding_box(msg->data.frame, kv.second.id().id(), rect, raster_center,
                                          object_img_radius);
        }
...
void ContactBlobCamera::add_false_positives(
    ...
    for (int i = 0; i < max_false_positives_; i++) {
        ...
        Eigen::Vector2d raster_center(parent_->random()->rng_uniform_int(0, img_width_),
        ...
        cv::Rect rect(std::floor(raster_center(0)) - std::floor(object_img_radius),
                      ...
                      std::floor(object_img_radius * 2) + 1, std::floor(object_img_radius * 2) + 1);
...
bool ContactBlobCamera::step() {
    ...
    sensor_frame.quat() =
        static_cast<sc::Quaternion>(parent_->state_truth()->quat() * this->transform()->quat());
    sensor_frame.pos() = this->transform()->pos() + parent_->state_truth()->pos();

    if (show_frustum_) {
        draw_frustum(frustum_shapes_, sensor_frame.quat().roll(), sensor_frame.quat().pitch(),
                     sensor_frame.quat().yaw());
    }
    ...
    if (show_sim_contacts_ && time_->t() > last_contact_send_time_ + contact_send_dt_) {
    ...
    if (show_image_) {
        std::string window_name = window_name_ + "-" + std::to_string(parent_->id().id()) + "-"
...
Eigen::Vector2d ContactBlobCamera::project_rel_3d_to_2d(Eigen::Vector3d rel_pos) {
    ...
    Eigen::Vector2d s(focal_length_ * -pos(0) / pos(2), focal_length_ * pos(1) / pos(2));
...
void ContactBlobCamera::draw_object_with_bounding_box(cv::Mat& frame, const int& id,
                                                      const cv::Rect& rect,
                                                      const Eigen::Vector2d& center,
                                                      const double& radius) {
    Eigen::Vector2i c(std::floor(center(0)), std::floor(center(1)));

    cv::circle(frame, cv::Point(c(0), c(1)), std::floor(radius), cv::Scalar(255, 255, 255), -1, 8,
               0);

    cv::rectangle(frame, rect, cv::Scalar(0, 0, 255), 1, 8, 0);

    cv::Point top_left(rect.x + rect.width, rect.y + rect.height);
    cv::putText(frame, std::to_string(id), top_left, cv::FONT_HERSHEY_SIMPLEX, 1.0,
                cv::Scalar(8, 100, 22), 1, 8, false);
}
