...
void ContactBlobCamera::init(std::map<std::string, std::string> &params) {
    ...
    if (get_vec_of_vecs(sim_det_str, vecs, ", ")) {
        ...
        for (auto &vec : vecs) {
            ...
            sc::StatePtr cnt_state = std::make_shared<sc::State>(Eigen::Vector3d(x, y, z), Eigen::Vector3d(0, 0, 0),
    ...
    if (log_detections_) {
        detections_file_.open(parent_->mp()->log_dir() + "/blob_sensor_detections_" + std::to_string(camera_id_) +
                              ".txt");
    }
...
void ContactBlobCamera::draw_frustum(const std::vector<scrimmage_proto::ShapePtr> &frustum_shapes, double x_rot,
    ...
    sc::path_to_lines(sensor_scene_bb, scene_bb_line, std::static_pointer_cast<EntityPlugin>(shared_from_this()));
...
void ContactBlobCamera::contacts_to_bounding_boxes(const scrimmage::State &sensor_frame,
    ...
    for (auto &kv : contacts) {
        ...
        cv::Rect rect(std::floor(raster_center(0)) - std::floor(object_img_half_width),
                      ...
                      std::floor(object_img_half_width * 2) + 1, std::floor(object_img_half_height * 2) + 1);

        if (object_img_radius > 0) {
            draw_object_with_bounding_box(msg->data.frame, kv.second.id().id(), rect, raster_center, object_img_radius);
        }
...
void ContactBlobCamera::add_false_positives(std::shared_ptr<scrimmage::Message<ContactBlobCameraType>> &msg) {
    for (int i = 0; i < max_false_positives_; i++) {
        ...
        if (object_img_radius > 0) {
            draw_object_with_bounding_box(msg->data.frame, id, rect, raster_center, object_img_radius);
        }
...
bool ContactBlobCamera::step() {
    ...
    sensor_frame.quat() = static_cast<sc::Quaternion>(parent_->state_truth()->quat() * this->transform()->quat());
    ...
    if (show_sim_contacts_ && time_->t() > last_contact_send_time_ + contact_send_dt_) {
        ...
        for (auto &kv : sim_contacts_) {
            ...
            Eigen::Vector3d sphere_center(kv.second.state()->pos().x(), kv.second.state()->pos().y(),
    ...
    if (show_image_) {
        std::string window_name =
            window_name_ + "-" + std::to_string(parent_->id().id()) + "-" + std::to_string(camera_id_);
...
Eigen::Vector2d ContactBlobCamera::project_rel_3d_to_2d(Eigen::Vector3d rel_pos) {
    ...
    Eigen::Vector2d ndc((s(0) + canvas_width_ / 2.0) / canvas_width_, (s(1) + canvas_height_ / 2.0) / canvas_height_);
...
void ContactBlobCamera::draw_object_with_bounding_box(cv::Mat &frame, const int &id, const cv::Rect &rect,
                                                      const Eigen::Vector2d &center, const double &radius) {
    Eigen::Vector2i c(std::floor(center(0)), std::floor(center(1)));

    cv::circle(frame, cv::Point(c(0), c(1)), std::floor(radius), cv::Scalar(255, 255, 255), -1, 8, 0);

    cv::rectangle(frame, rect, cv::Scalar(0, 0, 255), 1, 8, 0);

    cv::Point top_left(rect.x + rect.width, rect.y + rect.height);
    cv::putText(frame, std::to_string(id), top_left, cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(8, 100, 22), 1, 8,
                false);
}
