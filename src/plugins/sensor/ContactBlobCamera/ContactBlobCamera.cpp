...
void ContactBlobCamera::init(std::map<std::string, std::string> &params) {
  std::stringstream ss;
  ss << parent_->id().id();
  parameters_file_.open(parent_->mp()->log_dir() + "/blob_sensor_parameters_" +
                        ss.str() + ".txt");
...
void ContactBlobCamera::draw_frustum(
  ...
  if (frustum_shapes.size() != 8) {
    std::cerr << "ContactBlobCamera::draw_frustum: ERROR: input shape vector "
                 "must be size 8"
              << std::endl;
    return;
  }
  ...
  for (int idx_bb = 0; idx_bb != 4; ++idx_bb) {
    ...
    scrimmage::set(frustum_shapes.at(idx_bb)->mutable_line()->mutable_start(),
                   box_edge_start);
...
void ContactBlobCamera::contacts_to_bounding_boxes(
  ...
  for (auto &kv : contacts) {
    ...
    if (object_img_radius > 0) {
      draw_object_with_bounding_box(msg->data.frame, kv.second.id().id(), rect,
...
void ContactBlobCamera::add_false_positives(
  ...
  for (int i = 0; i < max_false_positives_; i++) {
    ...
    if (object_img_radius > 0) {
      draw_object_with_bounding_box(msg->data.frame, id, rect, raster_center,
                                    object_img_radius);
    }
