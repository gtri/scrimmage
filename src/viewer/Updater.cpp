...
void Updater::set_quat(const scrimmage::Quaternion &quat,
  ...
  actor->RotateWXYZ(sc::Angles::rad2deg(aa.angle()), aa.axis()(0), aa.axis()(1),
                    aa.axis()(2));
}
  ...
  for (int i = 0; i < shapes.shape_size(); i++) {
    ...
    switch (shape.oneof_type_case()) {
      ...
      case sp::Shape::kCircle:
        shape_status =
            draw_circle(new_shape, shape.circle(), actor, source, mapper);
        break;
      case sp::Shape::kCone:
        shape_status =
      ...
      case sp::Shape::kEllipse:
        shape_status =
            draw_ellipse(new_shape, shape.ellipse(), actor, source, mapper);
        break;
      case sp::Shape::kLine:
        shape_status =
      ...
      case sp::Shape::kPolydata:
        shape_status =
            draw_polydata(new_shape, shape.polydata(), actor, source, mapper);
        break;
      case sp::Shape::kPolygon:
        shape_status =
            draw_polygon(new_shape, shape.polygon(), actor, source, mapper);
        break;
      case sp::Shape::kPolyline:
        shape_status =
            draw_polyline(new_shape, shape.polyline(), actor, source, mapper);
        break;
      case sp::Shape::kSphere:
        shape_status =
            draw_sphere(new_shape, shape.sphere(), actor, source, mapper);
        break;
      case sp::Shape::kSpline:
        shape_status =
            draw_spline(new_shape, shape.spline(), actor, source, mapper);
        break;
      case sp::Shape::kText:
        shape_status =
      ...
      case sp::Shape::kTriangle:
        shape_status =
            draw_triangle(new_shape, shape.triangle(), actor, source, mapper);
        break;
      default:
        break;
    }
...
bool Updater::update_utm_terrain(
  ...
  if (utm->enable_terrain()) {
    ...
    if (utm->enable_extrusion()) {
      ...
      extrusion_reader->AddFeatureProperty(utm->extrusion_property().c_str(),
                                           defval);
      ...
      vtkSmartPointer<vtkDoubleArray> heights = vtkDoubleArray::SafeDownCast(
...
bool Updater::draw_ellipse(const bool &new_shape,
  ...
  if (new_shape) {
    ...
    // only the outline of the ellipse
    polygonSource->SetNumberOfSides(30);
