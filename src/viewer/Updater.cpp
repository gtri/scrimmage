        ...
        if (utm->enable_extrusion()) {
            ...
            for (vtkIdType n(0); n < numCells; n++) {
                ...
                for (vtkIdType m(0); m < cellPointIds->GetNumberOfIds(); m++) {
                    ...
                    if (m == 0) {
                        cellLocator
                            ->FindClosestPoint(p, closestPoint, cellId, subId, closestPointDist2);
                    }
...
bool Updater::draw_arc(
    const bool& new_shape,
...
void Updater::get_model_texture(
    std::string name,
    std::string& model_file,
    bool& model_found,
    std::string& texture_file,
