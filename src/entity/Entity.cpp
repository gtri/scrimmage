...
bool Entity::init(
  ...
  if (info.count("motion_model") > 0) {
    ...
    if (status.status == PluginStatus<MotionModel>::cast_failed) {
      cout << "Failed to open motion model plugin: " << info["motion_model"]
           << endl;
      return false;
  ...
  for (auto autonomy_name : autonomy_names) {
    auto autonomy = make_autonomy<Autonomy>(
        info[autonomy_name], plugin_manager, overrides[autonomy_name], parent,
        state_, id_to_team_map, id_to_ent_map, proj_, contacts, file_search,
        rtree, pubsub, time, param_server, plugin_tags, param_override_func,
        controllers_, debug_level);
...
bool Entity::parse_visual(std::map<std::string, std::string> &info,
  ...
  find_model_properties(it->second, cv_parse, *file_search_, overrides, visual_,
