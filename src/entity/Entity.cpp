...
State Entity::state_to_belief_frame(const State& other) {
    State s;
    Eigen::Vector3d new_local_pos = eigen_to_belief_frame(other.pos());
...
Eigen::Vector3d Entity::eigen_to_belief_frame(const Eigen::Vector3d& other_truth) {
    Eigen::Vector3d new_pos = state_belief_->pos() + (other_truth - state_truth_->pos());
    return new_pos;
}
