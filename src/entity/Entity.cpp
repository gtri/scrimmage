...
namespace scrimmage {
  ...
  bool Entity::init(const SimUtilsInfo& sim_info, EntityInitParams init_params) {
    ...
    bool init_empty_motion_model = true; // Still init a dummy motion model when using a gpu one. Otherwise 
                                         // program may segfault during execution.
    ...
    for (std::list<std::string>::reverse_iterator rit = controller_names.rbegin();
      ...
      if (connect_to_motion_model) {
        if (!gpu_motion_model && !verify_io_connection(controller->vars(), motion_model_->vars())) {
  ...
  if (motion_model_ != nullptr &&
      motion_model_->vars().input_variable_index().size() > 0 &&
      controllers_.size() == 0) {
