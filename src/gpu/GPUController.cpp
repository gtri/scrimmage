...
std::map<std::string, GPUMotionModelPtr> GPUController::build_motion_models(MissionParsePtr mp) {
    ...
    for (auto ent_descs : mp->entity_descriptions()) {
        ...
        if (mp->attributes().count(motion_model_name) == 0) {
            std::cerr << "No GPU Motion Model named \'" << motion_model_name
                      << "\' found in Mission File\n";
            continue;
        }
        std::map<std::string, std::string> motion_model_attributes =
            mp->attributes()[motion_model_name];
        KernelBuildOpts opts = get_kernel_build_opts(motion_model_attributes);
        std::string kernel_name =
            scrimmage::get<std::string>("kernel_name", motion_model_attributes, "");
        ...
        if (kernel_queue_opt.has_value()) {
            ...
            GPUMotionModelPtr motion_model =
                OpenCLUtils::make_gpu_motion_model(kernel_queue.first, kernel_queue.second, opts);
    ...
    std::transform(
        ...
        [](cl::Device& device) {
            ...
            if (OpenCLUtils::check_error(
                ...
                max_clock_freq =
                    std::numeric_limits<cl_uint>::min();  // Still need to return something, but
                                                          // will not picked as the device with the
