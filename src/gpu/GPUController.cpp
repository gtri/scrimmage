...
bool GPUController::KernelSource::EqualTo::operator()(
    const KernelSource& lhs,
...
KernelBuildOpts GPUController::get_kernel_build_opts(
    ...
    auto to_path = [&](std::string filename,
        ...
        if (!fs::exists(filename) && !kernel_dir.empty()) {
            // The kernel_dir is not part of the recursive iteration below. Explicitly
            // check if it matches
