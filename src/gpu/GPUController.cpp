...
GPUController::GPUController() {
  ...
  if (kernel_dir_str == nullptr) {
    ...
    std::transform(kernel_dirs.cbegin(),
                   kernel_dirs.cend(),
...
KernelBuildOpts GPUController::get_kernel_build_opts(
  ...
  auto to_path = [&](std::string filename, const fs::path& kernel_dir) -> std::optional<fs::path> {
...
std::map<std::string, GPUPluginBuildParams> GPUController::get_plugin_params(MissionParsePtr mp,
  ...
  for (auto ent_descs : mp->entity_descriptions()) {
    ...
    std::string plugin_name = descriptor[plugin_declaration];
    if (plugin_name == "") {
      continue;
    }
    ...
    KernelBuildOpts opts = get_kernel_build_opts(plugin_attributes);
    std::string kernel_name = scrimmage::get<std::string>("kernel_name", plugin_attributes, "");
    ...
    if (kernel_queue_opt.has_value()) {
      ...
      named_plugins.emplace(
          plugin_name, GPUPluginBuildParams{opts.single_precision, kernel_queue.first, kernel_queue.second});
    }
  }
...
cl::Program::Sources GPUController::read_kernels(const std::vector<fs::path>& kernel_src_dirs) {
  ...
  std::transform(
      kernel_srcs.cbegin(), kernel_srcs.cend(), std::back_inserter(ret_srcs), kernel_src_to_cl_src);
  return ret_srcs;
}
...
std::optional<cl::Device> GPUController::pick_device(const KernelBuildOpts& opts) {
  ...
  std::transform(usable_devices.begin(),
                 usable_devices.end(),
  ...
  auto max_it = std::max_element(
      clock_freq_devices.cbegin(),
      clock_freq_devices.cend(),
      [](const ClockFreqDevice& rhs, const ClockFreqDevice& lhs) { return rhs.first < lhs.first; });
