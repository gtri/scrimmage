...
boost::optional<std::string> FileSearch::find_mission(std::string mission, bool verbose) {
    ...
    std::list<std::string> env_vars{"SCRIMMAGE_MISSION_PATH", "/usr/share/scrimmage",
...
bool FileSearch::find_file(const std::string& search, std::string ext, const std::string& env_var,
                           std::string& result, bool verbose) {
    // Find the xml file.
    // Search order:
