...
boost::optional<std::string> FileSearch::find_mission(std::string mission,
    ...
    std::string out;
    auto search = [&](auto env) {
        return this->find_file(mission, "xml", env, out, verbose);
    };
    std::list<std::string> env_vars{"SCRIMMAGE_MISSION_PATH",
                                    "/usr/share/scrimmage",
                                    "/usr/local/share/scrimmage"};
    auto it = br::find_if(env_vars, search);
    return it == env_vars.end() ? boost::none
                                : boost::optional<std::string>(out);
}

bool FileSearch::find_file(const std::string &search, std::string ext,
    ...
    if (filenames.size() > 1) {
        std::cout << "==============================================="
                  << std::endl;
        std::cout << "WARNING: Multiple XML files with same name found"
                  << std::endl;
...
void FileSearch::find_files(
    ...
    if (env_var.find("/") == std::string::npos) {
        ...
        if (env_p == NULL) {
            std::cout << env_var << " environment variable not set"
                      << std::endl;
            return;
        }
    ...
    while (it != tok.end()) {
        auto starts_with = [&](std::string &s) {
            return (*it == s) ||
                   (boost::starts_with(s, *it) && s.size() > it->size() &&
    ...
    for (const std::string &t : tok) {
        ...
        if (fs::exists(root) && fs::is_directory(root)) {
            ...
            fs::recursive_directory_iterator fs_it(root);
            fs::recursive_directory_iterator endit;

            while (fs_it != endit) {
                fs::path path = fs_it->path();
                if (fs::is_regular_file(*fs_it) && path.extension() == ext) {
