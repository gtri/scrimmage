...
SimControl::SimControl()
    : id_to_team_map_(std::make_shared<std::unordered_map<int, int>>()),
      ...
      mp_(std::make_shared<MissionParse>()), time_(std::make_shared<Time>()),
      param_server_(std::make_shared<ParameterServer>()),
      global_services_(std::make_shared<GlobalService>()), timer_(Timer()),
      log_(std::make_shared<Log>()), printer_(std::make_shared<Print>()),
      ...
      sim_plugin_(std::make_shared<EntityPlugin>()), limited_verbosity_(false) {
  pause(false);
  prev_paused_ = false;
  single_step(false);
...
bool SimControl::generate_entity(const int &ent_desc_id,
  ...
  bool ent_status = ent->init(
      plugin_attr_map, params, id_to_team_map_, id_to_ent_map_, contacts_, mp_,
      proj_, id, ent_desc_id, plugin_manager_, file_search_, rtree_, pubsub_,
      printer_, time_, param_server_, global_services_, std::set<std::string>{},
...
bool SimControl::run_networks() {
  ...
  for (auto &kv : *networks_) {
    ...
    if (!result && kv.second->print_err_on_exit) {
      cout << "Network requested simulation termination: " << kv.second->name()
           << endl;
    }
    all_true &= result;
...
bool SimControl::start() {
  ...
  if (get("show_plugins", mp_->params(), false)) {
    plugin_manager_->print_plugins("scrimmage::Autonomy", "Autonomy Plugins",
                                   *file_search_);
    plugin_manager_->print_plugins("scrimmage::MotionModel", "Motion Plugins",
                                   *file_search_);
    ...
    plugin_manager_->print_plugins("scrimmage::EntityInteraction",
                                   "Entity Interaction Plugins", *file_search_);
  ...
  if (mp_->params().count("stream_port") > 0 &&
    ...
    if (mp_->network_gui()) {
      ...
      network_thread_ = std::thread(
          &Interface::init_network, &(*incoming_interface_), Interface::server,
  ...
  auto gen_ent_cb = [&](auto &msg) {
    ...
    if (not this->generate_entity(it_ent_desc_id->second, params,
                                  plugin_attr_map)) {
      cout << "Failed to generate entity with tag: " << msg->data.entity_tag()
           << endl;
      return;
    }
  };
  sim_plugin_->subscribe<sm::GenerateEntity>("GlobalNetwork", "GenerateEntity",
                                             gen_ent_cb);
...
bool SimControl::end_condition_reached() {
  ...
  if (ents_.empty()) {
    ...
    if (end_conditions_.count(EndConditionFlags::ALL_DEAD) ||
      ...
      std::cout << std::endl
                << "End of Simulation: No Entities Remaining" << std::endl;
      return true;
    }
    ...
    const bool all_same_team =
        std::all_of(ents_.rbegin(), ents_.rend(), [&](EntityPtr &ent) {
  ...
  while (true) {
    ...
    while (true) {
      ...
      if (task_type == Task::Type::AUTONOMY) {
        ...
        auto run = [&](auto &a) {
          return a->step_loop_timer(temp_dt) ? a->step_autonomy(temp_t, temp_dt)
                                             : true;
        };
