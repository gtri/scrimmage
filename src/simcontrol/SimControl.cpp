...
void SimControl::set_enable_gui(bool enable) { mp_->set_enable_gui(false); }

void SimControl::display_progress(const bool &enable) { display_progress_ = enable; }
...
bool SimControl::start() {
  ...
  if (mp_->params().count("stream_port") > 0 && mp_->params().count("stream_ip") > 0) {
    if (mp_->network_gui()) {
      outgoing_interface_->init_network(
          Interface::client, mp_->params()["stream_ip"], std::stoi(mp_->params()["stream_port"]));
  ...
  auto gen_ent_cb = [&](auto &msg) {
    ...
    if (it_ent_desc_id == mp_->entity_tag_to_id().end()) {
      cout << "ERROR: Failed to find entity_tag, " << msg->data.entity_tag() << ", in mission file."
           << endl;
      return;
    }
    ...
    for (int i = 0; i < msg->data.plugin_param().size(); i++) {
      plugin_attr_map[msg->data.plugin_param(i).plugin_type()]
                     [msg->data.plugin_param(i).tag_name()] = msg->data.plugin_param(i).tag_value();
    }
  ...
  if (entity_thread_types_.count(Task::Type::AUTONOMY)) {
    ...
    for (EntityPtr &ent : ents_) {
      for (auto a : ent->autonomies()) {
        success &= exec_step(
            a, [&](auto a) { return a->step_loop_timer(dt_) ? a->step_autonomy(t_, dt_) : true; });
      }
    }
  }
  ...
  for (int i = 0; i < mp_->motion_multiplier(); i++) {
    ...
    for (EntityPtr &ent : ents_) {
      for (auto c : ent->controllers()) {
        success &=
            exec_step(c, [&](auto c) { return c->step_loop_timer(dt_) ? c->step(t_, dt_) : true; });
      }
    }
  }
#if ENABLE_GPU_ACCELERATION == 1
