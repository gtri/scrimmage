...
bool Log::init(const std::string &dir, Log::Mode mode) {
  ...
  if (mode_ == WRITE && drop_bin_logging_ == false) {
    if (open_file(frames_name_, frames_fd_)) {
      frames_output_ =
          std::make_shared<google::protobuf::io::FileOutputStream>(frames_fd_);
    }
    if (open_file(shapes_name_, shapes_fd_)) {
      shapes_output_ =
          std::make_shared<google::protobuf::io::FileOutputStream>(shapes_fd_);
    }
    if (open_file(utm_terrain_name_, utm_terrain_fd_)) {
      utm_terrain_output_ =
          std::make_shared<google::protobuf::io::FileOutputStream>(
              utm_terrain_fd_);
    }
...
bool Log::parse(std::string dir) {
  ...
  if (!fs::exists(fs::path(contact_visual_name_))) {
    cout << "Contact Visual file doesn't exist: " << contact_visual_name_
         << endl;
  } else {
...
bool Log::parse(std::string filename, Log::FileType type) {
  ...
  if (!input) {
    cout << "Failed to open FileInputStream. Filename: " << filename.c_str()
         << endl;
  }
...
bool Log::parse_frames(std::string filename, ZeroCopyInputStreamPtr input) {
  ...
  do {
    std::shared_ptr<scrimmage_proto::Frame> frame =
        std::make_shared<scrimmage_proto::Frame>();
...
bool Log::parse_contact_visual(std::string filename,
  ...
  do {
    ...
    success =
        this->readDelimitedFrom(filename, input, contact_visual, clean_eof);
    if (clean_eof || !success) break;
...
bool Log::close_fileoutputstream(ZeroCopyOutputStreamPtr stream) {
  ...
  if (stream) {
    status =
        dynamic_cast<google::protobuf::io::FileOutputStream &>(*stream).Close();
  }
  return status;
}
