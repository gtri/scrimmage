...
bool Log::parse_utm_terrain(std::string filename,
    ...
    do {
        ...
        success =
            this->readDelimitedFrom(filename, input, utm_terrain, clean_eof);
        if (clean_eof || !success) break;
...
bool Log::parse_contact_visual(std::string filename,
    ...
    if (!clean_eof) {
        cout << "ContactVisual - WARNING: Clean end-of-file not detected."
             << endl;
    }
    return true;
}
...
bool Log::close_fileoutputstream(ZeroCopyOutputStreamPtr stream) {
    ...
    bool status = false;
    if (stream) {
        status = dynamic_cast<google::protobuf::io::FileOutputStream &>(*stream)
                     .Close();
    }
    return status;
}
